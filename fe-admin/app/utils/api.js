...
export function getLogoUrl(url) {
  ...
  // Direct S3 optimization for known folders
  const s3Base = process.env.NEXT_PUBLIC_S3_BASE_URL ? process.env.NEXT_PUBLIC_S3_BASE_URL.replace(/\/$/, "") : "https://is3.cloudhost.id/satwaid-bucket";
  const s3Folders = ['/listings', '/avatars', '/logos', '/payments', '/shipments', '/disbursements', '/advertisements', '/complaints'];
...
export function getImageUrl(path) {
  ...
  const s3Base = process.env.NEXT_PUBLIC_S3_BASE_URL ? process.env.NEXT_PUBLIC_S3_BASE_URL.replace(/\/$/, "") : "https://is3.cloudhost.id/satwaid-bucket";
  const s3Folders = ['/listings', '/avatars', '/logos', '/payments', '/shipments', '/disbursements', '/advertisements', '/complaints'];
  const matchedFolder = s3Folders.find(folder => formattedPath.startsWith(folder));
  if (matchedFolder) {
    return `${s3Base}${formattedPath}`;
  }
