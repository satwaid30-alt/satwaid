...
export default function AdminTransactionPage() {
  ...
  return (
    <div className="p-6 lg:p-10 space-y-10 min-h-screen">
      ...
      {/* Transactions Table */}
      <div className="bg-zinc-900/30 border border-zinc-800 rounded-[2.5rem] overflow-hidden relative">
                ...
                currentItems.map((order, index) => (
                  <tr key={order.id} className="hover:bg-zinc-800/20 transition-colors group">
                    ...
                    <td className="px-6 py-6">
                      <div className="space-y-1">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span className="px-1.5 py-0.5 bg-zinc-800 text-[8px] text-zinc-400 rounded font-black uppercase tracking-widest border border-zinc-700">ID: {order.product?.product_id || "-"}</span>
                        </div>
                    ...
                    <td className="px-6 py-6 text-center font-mono">
                      <span className="text-xs font-black text-zinc-400">{order.items && order.items.length > 0 ? order.items.reduce((sum, item) => sum + (item.quantity || 1), 0) : order.quantity || 1}</span>
                    </td>
                    ...
                    <td className="px-6 py-6">
                      <div className="flex justify-center">
                        {order.payment_proof ? (
                          <button onClick={() => setSelectedProof(getImageUrl(order.payment_proof))} className="p-2.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 rounded-xl hover:bg-emerald-500 hover:text-zinc-950 transition-all flex items-center gap-2">
                            <ImageIcon size={16} />
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1">
                {[...Array(totalPages)].map((_, i) => (
                  <button key={i + 1} onClick={() => setCurrentPage(i + 1)} className={`w-8 h-8 rounded-xl text-[10px] font-black transition-all border ${currentPage === i + 1 ? "bg-emerald-500 border-emerald-400 text-zinc-950" : "bg-zinc-950 border-zinc-800 text-zinc-500 hover:text-white hover:border-zinc-700"}`}>
                    {i + 1}
                  </button>
                ))}
              </div>
      ...
      {selectedProof && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="relative max-w-4xl w-full bg-zinc-900 rounded-[2rem] border border-zinc-800 overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="flex items-center justify-between p-6 border-b border-zinc-800">
              <h3 className="text-sm font-black text-white uppercase tracking-widest flex items-center gap-3">
                <ImageIcon size={18} className="text-emerald-500" /> Bukti Pembayaran Pembeli
              </h3>
              <button onClick={() => setSelectedProof(null)} className="p-2 bg-zinc-800 hover:bg-red-500 text-zinc-400 hover:text-white rounded-xl transition-all">
                <XCircle size={20} />
              </button>
            </div>
            <div className="p-4 bg-zinc-950 flex items-center justify-center min-h-[400px]">
              <img src={selectedProof} alt="Payment Proof" className="max-h-[70vh] object-contain rounded-xl" />
            </div>
            <div className="p-6 bg-zinc-900 border-t border-zinc-800 flex justify-end gap-4">
              <a href={selectedProof} target="_blank" rel="noreferrer" className="px-6 py-3 bg-zinc-800 hover:bg-zinc-700 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all">
                <ExternalLink size={14} /> Buka Tab Baru
              </a>
              <button onClick={() => setSelectedProof(null)} className="px-8 py-3 bg-emerald-500 hover:bg-emerald-400 text-zinc-950 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all">
                Selesai
              </button>
            </div>
          </div>
      ...
      {cancelOrderData && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-zinc-900 border border-zinc-800 w-full max-w-md rounded-[2.5rem] p-10 relative z-10 animate-in zoom-in duration-300">
            <div className="flex flex-col items-center text-center space-y-6">
              ...
              <div className="w-full text-left space-y-2">
                ...
                <textarea required rows={3} value={cancelReason} onChange={(e) => setCancelReason(e.target.value)} className="w-full bg-zinc-950 border border-zinc-800 rounded-2xl py-4 px-6 text-white text-xs focus:outline-none focus:border-red-500 transition-all resize-none font-bold" placeholder="Masukkan alasan pembatalan..." />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 w-full pt-4">
                <button
                  ...
                  className="py-4 bg-zinc-800 hover:bg-zinc-700 text-zinc-400 font-bold text-xs uppercase tracking-wider rounded-2xl transition-all"
                >
                  Batal
                </button>
                <button onClick={handleCancelOrder} disabled={isCancelling || !cancelReason.trim()} className="py-4 bg-red-500 hover:bg-red-400 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold text-xs uppercase tracking-wider rounded-2xl transition-all flex items-center justify-center gap-2">
                  {isCancelling ? (
                    <>
