...
import { Shield, Eye, EyeOff, Check, AlertTriangle, Lock } from "lucide-react";
    ...
    <div className="space-y-1.5">
      ...
      <div className="relative">
        ...
        <input
          ...
          className={`w-full bg-zinc-950 border rounded-[10px] pl-12 pr-12 py-3.5 text-white placeholder:text-zinc-600 focus:outline-none focus:ring-2 transition-all ${error ? "border-red-500/60 focus:ring-red-500/30" : "border-zinc-700 focus:ring-emerald-500 focus:border-transparent"}`}
        />
        <button type="button" onClick={onToggleShow} className="absolute right-4 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-white transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <div className="bg-zinc-900 border border-zinc-800 rounded-[10px] p-6 md:p-8">
            <h2 className="text-lg font-bold text-white mb-6 flex items-center gap-2">Ubah Password</h2>

            <form onSubmit={handleSubmit} className="space-y-5">
              {/* Password Saat Ini */}
              <PasswordInput label="Password Saat Ini" name="current_password" value={form.current_password} onChange={handleChange} showPass={show.current} onToggleShow={() => toggleShow("current")} placeholder="Masukkan password aktif Anda sekarang" hint="" error={errors.current_password} />

              <div className="border-t border-zinc-800" />

              {/* Password Baru */}
              <PasswordInput label="Password Baru" name="new_password" value={form.new_password} onChange={handleChange} showPass={show.new} onToggleShow={() => toggleShow("new")} placeholder="Minimal 6 karakter, bebas apapun" hint="Bebas menggunakan huruf, angka, atau karakter apapun." error={errors.new_password} />
              ...
              {form.new_password && strength && (
                <div className="space-y-1.5 -mt-2">
                  ...
                  <p className={`text-xs font-semibold ${strength.textColor}`}>Kekuatan: {strength.label}</p>
                </div>
              )}

              {/* Konfirmasi Password */}
              <PasswordInput label="Konfirmasi Password Baru" name="confirm_password" value={form.confirm_password} onChange={handleChange} showPass={show.confirm} onToggleShow={() => toggleShow("confirm")} placeholder="Ulangi password baru Anda" hint="" error={errors.confirm_password} />
              ...
              <div className="pt-2">
                <button type="submit" disabled={isLoading} className={`w-full flex items-center justify-center gap-2 py-3.5 bg-[#228B22] hover:bg-[#4CBB17] text-white font-bold rounded-[10px] transition-all ${isLoading ? "opacity-70 cursor-not-allowed" : ""}`}>
                  {isLoading ? (
        ...
        <div className="space-y-4">
          ...
          <div className="bg-amber-500/5 border border-amber-500/20 rounded-[10px] p-6">
            ...
            <p className="text-xs text-zinc-500 mb-3">Jika tidak ingat password saat ini, gunakan fitur lupa password di halaman login untuk mendapat password sementara.</p>
            <a href="/login" className="text-xs font-bold text-amber-400 hover:text-amber-300 transition-colors underline underline-offset-2">
              Ke halaman login →
            </a>
          </div>
        </div>
