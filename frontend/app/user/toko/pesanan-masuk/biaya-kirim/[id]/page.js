...
export default function InputShippingCostPage({ params }) {
        ...
        try {
            ...
            if (res.ok && result.data) {
                ...
                setCostForm({
                    ...
                    packing_cost: result.data.packing_cost || ""});
            } else {
                setModalConfig({
                    isOpen: true,
                    type: "warning",
    ...
    const handleCostSubmit = (e) => {
        ...
        // Validation
        if (!order.product?.is_free_shipping && (Number(costForm.shipping_cost) <= 0)) {
            setModalConfig({
                isOpen: true,
                type: "warning",
    ...
    const processUpdateCost = async () => {
        ...
        try {
            const payload = {
                shipping_cost: order.product?.is_free_shipping ? 0 : (parseInt(costForm.shipping_cost) || 0),
                packing_cost: order.product?.is_free_packing ? 0 : (parseInt(costForm.packing_cost) || 0)
            };

            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/orders/${id}/shipping-cost`, {
                method: 'PUT',
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                    ...
                    <span className="px-3 py-1 bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 rounded-full text-[10px] font-black uppercase tracking-widest">
                        {order.status === 'waiting_shipping_cost' ? "Perlu Input Ongkir" : "Invoice Terkirim"}
                    </span>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div className="lg:col-span-7 space-y-8">
                                ...
                                <div className="bg-zinc-950/50 rounded-[2.5rem] p-8 border border-zinc-800 space-y-4">
                                    ...
                                    <div className="flex justify-between items-center text-sm border-b border-zinc-800/20 pb-3">
                                        ...
                                        <span className="text-white font-black">
                                            {order.product?.is_free_shipping ? (
                                                <span className="text-emerald-500 uppercase text-[10px] font-black">Gratis</span>
                                            ) : (
                                    ...
                                    <div className="flex justify-between items-center text-sm border-b border-zinc-800/50 pb-4">
                                        <span className="text-zinc-500 font-bold">Biaya Admin Marketplace</span>
                                    ...
                                    <div className="pt-6 border-t border-zinc-800/50 flex flex-col sm:flex-row gap-4">
                                        ...
                                        <button
                                            ...
                                            className="flex-[2] py-4 lg:py-5 bg-emerald-500 hover:bg-emerald-400 text-zinc-950 font-black rounded-2xl transition-all shadow-xl shadow-emerald-500/20 disabled:opacity-50 flex items-center justify-center gap-3 active:scale-95 text-sm uppercase tracking-widest"
                                        >
                                            {isUpdatingCost ? (
                                                <>
                    ...
                    <div className="bg-zinc-900 border border-zinc-800 rounded-[2rem] lg:rounded-[3rem] p-6 lg:p-8 shadow-2xl space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 p-5 lg:p-6 bg-zinc-950/50 rounded-[1.5rem] lg:rounded-[2rem] border border-zinc-800">
                ...
                <div className="lg:col-span-5 space-y-8">
                    <div className="bg-zinc-900 border border-zinc-800 rounded-[2rem] lg:rounded-[3rem] p-6 lg:p-8 shadow-2xl space-y-8 lg:sticky lg:top-24">
                        <h3 className="text-[10px] font-black text-zinc-500 uppercase tracking-widest flex items-center gap-2">
                        ...
                        <div className="space-y-4 pt-6 border-t border-zinc-800">
                            ...
                            <div className="p-4 bg-amber-500/5 border border-dashed border-amber-500/20 rounded-2xl flex gap-3">
                                ...
                                <p className="text-[11px] text-amber-500/80 font-medium leading-relaxed italic">
                                    Pastikan Anda telah mengecek tarif kurir sesuai dengan dimensi & berat paket ke alamat tujuan di samping.
                                </p>
                            </div>
                        </div>
                    </div>
