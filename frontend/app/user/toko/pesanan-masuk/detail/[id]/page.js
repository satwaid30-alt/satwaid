    ...
    <div className="max-w-7xl mx-auto pt-2 pb-10 lg:py-10 animate-in fade-in slide-in-from-bottom-4 duration-500 px-4 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-[10px] overflow-hidden">
            <button onClick={() => setIsProductCardOpen(!isProductCardOpen)} className="w-full flex items-center justify-between p-4 md:p-8 hover:bg-zinc-800/50 transition-all border-b border-zinc-800/50">
              <div className="flex items-center gap-3 md:gap-4">
                <div className={`w-10 h-10 md:w-12 md:h-12 rounded-[10px] flex items-center justify-center transition-all shrink-0 ${isProductCardOpen ? "bg-emerald-500 text-zinc-950" : "bg-zinc-800 text-zinc-500"}`}>
                      ...
                      <div key={item.id || idx} className="bg-zinc-950/40 border border-zinc-800/80 rounded-[10px] p-6 flex flex-col md:flex-row items-center gap-6 group hover:border-emerald-500/20 transition-all">
                        ...
                        <div className="flex-1 min-w-0 text-center md:text-left space-y-1.5">
                          ...
                          <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                            ...
                            {item.product?.is_free_shipping && <span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-400 text-[9px] font-black uppercase tracking-wider rounded border border-emerald-500/20">Gratis Ongkir</span>}
                            {item.product?.is_free_packing && <span className="px-2 py-0.5 bg-blue-500/10 text-blue-400 text-[9px] font-black uppercase tracking-wider rounded border border-blue-500/20">Gratis Packing</span>}
                          </div>
                    ...
                    <div className="flex-1 space-y-4">
                      <div>
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          ...
                          {order.product?.is_free_shipping && <span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-400 text-[9px] font-black uppercase tracking-widest rounded border border-emerald-500/20">Gratis Ongkir</span>}
                          {order.product?.is_free_packing && <span className="px-2 py-0.5 bg-blue-500/10 text-blue-400 text-[9px] font-black uppercase tracking-widest rounded border border-blue-500/20">Gratis Packing</span>}
                        </div>
        ...
        <div className="lg:col-span-4 space-y-6 sticky top-24">
          <div className="bg-zinc-900 border border-zinc-800 rounded-[10px] p-4 md:p-8 space-y-6 md:space-y-8">
                  ...
                  <>
                    <div className="flex justify-between items-start text-sm">
                      <div className="flex flex-col text-left">
                        ...
                        {freeShippingCount > 0 && <span className="text-[10px] text-emerald-500 font-bold mt-0.5">({freeShippingCount} Produk Gratis Ongkir)</span>}
                      </div>
                      <span className="text-white font-black">
                        {(order.items && order.items.length > 0 ? order.items.every((item) => item.product?.is_free_shipping) : !!order.product?.is_free_shipping) || (order.shipping_cost !== null && parseFloat(order.shipping_cost) === 0 && !["pending_shipping_info", "waiting_shipping_cost"].includes(order.status)) ? (
                          ...
                          <span className="text-zinc-555 font-bold uppercase text-[10px]">Belum Ditentukan</span>
                        ) : (
                    ...
                    <div className="flex justify-between items-start text-sm">
                      <div className="flex flex-col text-left">
                        ...
                        {freePackingCount > 0 && <span className="text-[10px] text-emerald-500 font-bold mt-0.5">({freePackingCount} Produk Gratis Packing)</span>}
                      </div>
                      <span className="text-white font-black">
            ...
            <div className="space-y-3 pt-4">
              {/* {order.status === "waiting_shipping_cost" && (
                <Link href={`/user/toko/pesanan-masuk/biaya-kirim/${order.id}`} className="w-full py-4 bg-[#2563EB] hover:bg-[#1D4ED8] text-[#FFFFFF] font-black rounded-[10px] transition-all flex items-center justify-center gap-3 active:scale-95">
                  Input Ongkir & Packing
                </Link>
              )}

              {["waiting_shipment", "payment_verified"].includes(order.status) && (
                <Link href={`/user/toko/pesanan-masuk/pengiriman/${order.id}`} className="w-full py-5 bg-[#2563EB] hover:bg-[#1D4ED8] text-[#FFFFFF] rounded-[10px] transition-all font-black text-sm flex items-center justify-center gap-3 active:scale-95">
              ...
              <button
                ...
                className="w-full py-4 text-white bg-[#25D366] hover:bg-[#128C7E] rounded-[10px] transition-all font-black text-xs flex items-center justify-center gap-2.5 active:scale-95"
              >
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-[10px] p-4 md:p-8 space-y-6">
            <h3 className="text-xs font-black text-zinc-500 uppercase tracking-widest flex items-center gap-2">
