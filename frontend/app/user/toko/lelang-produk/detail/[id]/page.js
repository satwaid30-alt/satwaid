                        ...
                        <div className="w-full aspect-square bg-zinc-950 relative group cursor-zoom-in">
                            ...
                            {parsedImages.length > 1 && (
                                <div className="absolute inset-x-4 top-1/2 -translate-y-1/2 flex justify-between opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                                    <button
                                        onClick={() => setActiveImageIndex((prev) => (prev > 0 ? prev - 1 : parsedImages.length - 1))}
                                        className="p-3 lg:p-4 bg-zinc-900/80 backdrop-blur-md text-white rounded-xl lg:rounded-2xl hover:bg-amber-500 hover:text-zinc-950 transition-all pointer-events-auto"
                                    >
                                        <ChevronLeft size={20} className="lg:w-6 lg:h-6" />
                                    </button>
                                    <button
                                        onClick={() => setActiveImageIndex((prev) => (prev < parsedImages.length - 1 ? prev + 1 : 0))}
                                        className="p-3 lg:p-4 bg-zinc-900/80 backdrop-blur-md text-white rounded-xl lg:rounded-2xl hover:bg-amber-500 hover:text-zinc-950 transition-all pointer-events-auto"
                                    >
                            ...
                            <div className="p-4 lg:p-6 bg-zinc-950/30 border-t border-zinc-800 flex gap-3 lg:gap-4 overflow-x-auto custom-scrollbar">
                                {parsedImages.map((img, idx) => (
                                    <button
                                        ...
                                        className={`w-16 h-16 lg:w-20 lg:h-20 rounded-xl lg:rounded-2xl overflow-hidden border-2 shrink-0 transition-all duration-300 ${activeImageIndex === idx ? 'border-amber-500 scale-105' : 'border-zinc-800 opacity-40 hover:opacity-100'}`}
                                    >
                        ...
                        <div className="relative space-y-8">
                            <div className="space-y-4">
                                <div className="flex flex-wrap items-center gap-3">
                                    <span className="px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] border bg-amber-500/10 text-amber-500 border-amber-500/20">
                                        Lelang / Auction
                                    </span>
                            ...
                            {/* Price Card */}
                            <div className="rounded-3xl p-8 border group/price relative overflow-hidden shadow-2xl bg-amber-500/5 border-amber-500/20">
                                <div className="absolute top-0 right-0 p-8 transition-colors text-amber-500/5 group-hover/price:text-amber-500/10">
                                ...
                                <div className="relative">
                                    ...
                                    <div className="flex flex-wrap items-baseline gap-2 lg:gap-4">
                                        <p className="text-2xl lg:text-3xl font-black text-white tracking-tighter">
                                            {formatPrice(bids && bids.length > 0 ? bids[0].bid_amount : listing.start_bid)}
                                        </p>
                                ...
                                return (
                                    <div className="bg-zinc-950 border border-zinc-800 rounded-3xl p-6 lg:p-8 space-y-4">
                                        <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
                                            ...
                                            <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${isEnded ? 'bg-zinc-800 text-zinc-400' : 'bg-amber-500/10 text-amber-500 border border-amber-500/20'}`}>
                                            ...
                                            bids && bids.length > 0 ? (
                                                <div className="space-y-4">
                                                    ...
                                                    {listing.latestOrderUuid ? (
                                                        <Link
                                                            href={`/user/toko/pesanan-masuk/detail/${listing.latestOrderUuid}`}
                                                            className="inline-flex w-full h-11 bg-emerald-500 hover:bg-emerald-600 text-zinc-950 font-black rounded-2xl text-xs transition-all items-center justify-center gap-2 shadow-lg shadow-emerald-500/20 active:scale-95 cursor-pointer"
                                                        >
                                                            Lihat Detail Transaksi Pesanan
                                                        </Link>
                                                    ) : (
                                                        <p className="text-[10px] text-amber-500 font-bold text-center bg-amber-500/5 border border-amber-500/10 py-2 px-4 rounded-xl leading-relaxed">
                                                            Menunggu pemenang melakukan checkout pembayaran.
                                                        </p>
                                                    )}
                                                </div>
                                            ) : (
                                                <div className="py-4 text-center">
                                                    <p className="text-xs font-black text-zinc-500 uppercase tracking-wider bg-zinc-900 border border-zinc-800 py-3 px-4 rounded-2xl">
                                                        Tidak ada pemenang
                                                    </p>
                                                </div>
                                            )
                                                    ...
                                                    {bids.map((b, idx) => (
                                                        <div key={b.id || idx} className={`flex items-center justify-between p-3 rounded-xl border transition-all ${idx === 0 ? 'bg-amber-500/5 border-amber-500/20 shadow-md' : 'bg-zinc-900/30 border-zinc-800'}`}>
                                                            <div className="flex items-center gap-2 min-w-0">
                                                                <span className={`text-[10px] font-black w-5 h-5 rounded-full flex items-center justify-center ${idx === 0 ? 'bg-amber-500 text-zinc-950' : 'bg-zinc-800 text-zinc-400'}`}>
                                                                    {idx + 1}
                                                                </span>
                            ...
                            <div className="pt-4 lg:pt-6 flex flex-col sm:flex-row gap-3 lg:gap-4">
                                {isSold ? (
                                    ...
                                    <div className="flex-1 bg-amber-500/5 border border-amber-500/20 text-amber-400 font-black py-4 lg:py-5 rounded-2xl lg:rounded-[2rem] flex items-center justify-center gap-3 cursor-not-allowed text-xs lg:text-sm">
                                        <Gavel size={20} className="animate-pulse" />
                                        LELANG SEDANG BERLANGSUNG
                                    </div>
                                ) : (
                                    <Link
                                        href={`/user/toko/lelang-produk/edit/${listing.id}`}
                                        className="flex-1 font-black py-4 lg:py-5 bg-amber-500 hover:bg-amber-400 text-zinc-950 rounded-2xl lg:rounded-[2rem] flex items-center justify-center gap-3 transition-all active:scale-95 group/btn text-xs lg:text-sm cursor-pointer shadow-lg shadow-amber-500/20"
                                    >
