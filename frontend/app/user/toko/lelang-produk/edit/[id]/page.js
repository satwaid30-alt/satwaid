...
import { Gavel, ChevronRight, ChevronDown, ChevronUp, ChevronLeft, CheckCircle2, X, Image as ImageIcon, Truck, AlertCircle, Calendar, Coins, ScrollText, Plus, Tag, RotateCcw, Loader2, ArrowLeft } from "lucide-react";
...
export default function EditLelangListingPage({ params }) {
  ...
  const fetchListingData = async () => {
    try {
      ...
      if (res.ok && result.data) {
        ...
        let calculatedDuration = Math.round(diffTime / (1000 * 60 * 60 * 24)) || "1";
  ...
  const handleUpdateAuction = async (e) => {
    ...
    if (!isAgreed) {
      alert("Anda harus menyetujui pernyataan tanggung jawab di bawah sebelum menyimpan perubahan.");
      return;
    }

    // BIN Price Validation
    if (reptileData.bin_price) {
      const bin = parseInt(reptileData.bin_price.toString().replace(/\D/g, ""), 10);
      const ob = parseInt(reptileData.start_bid.toString().replace(/\D/g, ""), 10);
      if (bin <= ob) {
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight flex items-center gap-3">
            {isReauction ? <RotateCcw className="text-emerald-500 shrink-0" size={32} /> : <Gavel className="text-amber-500 shrink-0" size={32} />}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8 text-sm text-zinc-300">
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-3">
                  ...
                  <ul className="list-disc list-outside ml-9 text-zinc-400 space-y-2 leading-relaxed">
                    <li>Harga harus jelas and sesuai dengan produk yang ditawarkan.</li>
        ...
        <form onSubmit={handleUpdateAuction} className="px-0 py-6 sm:p-8 space-y-8">
          ...
          <div className="pt-8 border-t border-zinc-800">
            <label className="flex items-start gap-4 cursor-pointer group mb-8 px-2">
              ...
              <span className="text-xs font-bold text-zinc-500 group-hover:text-zinc-300 transition-colors leading-relaxed">Saya menyatakan bahwa data perubahan lelang di atas adalah benar. Saya bertanggung jawab penuh atas perubahan lelang ini.</span>
            </label>
            ...
            <div className="flex gap-3">
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    {isReauction ? <RotateCcw size={24} className="group-hover:rotate-45 transition-transform" /> : <Gavel size={24} className="group-hover:rotate-12 transition-transform" />}
            ...
            <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest text-center mt-4 flex items-center justify-center gap-2">
              ...
              Apabila melakukan perubahan maka produk akan di verifikasi ulang oleh admin
            </p>
          </div>
        </form>
      </div>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-[10px] w-full max-w-md p-10 text-center relative z-10 animate-in zoom-in-95 duration-300">
            ...
            <h3 className="text-2xl font-black text-white mb-3">{isReauction ? "Konfirmasi Mulai Ulang Lelang" : "Konfirmasi Perubahan"}</h3>
            <p className="text-zinc-400 mb-8 leading-relaxed">
              {isReauction ? (
                <>
                  Peringatan: Lelang ini akan dimulai ulang sebagai siklus baru dan akan <span className="text-amber-500 font-bold">diverifikasi ulang</span> oleh admin. Apakah Anda yakin?
                </>
              ) : (
                <>
                  Peringatan: Apabila melakukan perubahan maka produk lelang akan <span className="text-amber-500 font-bold">diverifikasi ulang</span> oleh admin. Apakah Anda yakin ingin melanjutkan?
                </>
              )}
            </p>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-[10px] w-full max-w-md p-12 text-center relative z-10 shadow-[0_0_100px_rgba(245,158,11,0.1)] animate-in zoom-in-95 duration-300">
            ...
            <h3 className="text-3xl font-black text-white mb-4">{isReauction ? "Lelang Dimulai Ulang!" : "Berhasil Disimpan!"}</h3>
            <p className="text-zinc-400 mb-10 leading-relaxed font-medium">
              {isReauction ? (
                <>
                  Pengajuan lelang kembali Anda telah berhasil dikirim dan sedang menunggu <span className="text-amber-500 font-bold">verifikasi ulang</span> oleh admin.
                </>
              ) : (
                <>
                  Perubahan produk lelang Anda telah berhasil disimpan dan sedang menunggu <span className="text-amber-500 font-bold">verifikasi ulang</span> oleh admin.
                </>
              )}
            </p>
