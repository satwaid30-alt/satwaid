...
export default function LelangProdukPage() {
    ...
    const resetForm = () => {
        setReptileData({ ...initialReptileData });
        setIsAgreed(false);
    };
    ...
    const handleAddAuction = async (e) => {
        ...
        try {
            ...
            if (!reptileData.name || !reptileData.species || !reptileData.start_bid || 
                !reptileData.multiple || !reptileData.end_date || !cleanDescription || 
                !reptileData.sex || !cleanShipping || !reptileData.shipping_type || reptileData.images.length === 0) {
            ...
            const payload = {
                ...
                user_id: userData.id,
                price: null,
                start_bid: (reptileData.start_bid || null),
                multiple: (reptileData.multiple || null),
                end_date: (reptileData.end_date || null),
            };
        ...
        <div className="max-w-5xl mx-auto py-10 space-y-10 animate-in fade-in duration-500">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-zinc-500 mt-1 font-bold uppercase tracking-widest text-[10px] sm:text-xs">Buka lelang untuk produk/reptil koleksi terbaik Anda</p>
                </div>
                <Link
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-3xl overflow-hidden shadow-2xl">
                <form onSubmit={handleAddAuction} className="p-8 space-y-8">
                    {/* Basic Info Section */}
                    <div className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label className="text-xs font-black text-zinc-500 uppercase tracking-widest ml-1">Nama Produk / Judul Lelang <span className="text-red-500">*</span></label>
                                <input
                                    type="text"
                                    required
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-black text-zinc-500 uppercase tracking-widest ml-1">Kategori / Spesies <span className="text-red-500">*</span></label>
                                <select
                                    required
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="space-y-2">
                                <label className="text-xs font-black text-zinc-500 uppercase tracking-widest ml-1 flex items-center gap-1.5">
                                    <Coins size={12} className="text-amber-500" /> Open Bid (OB) <span className="text-red-500">*</span>
                                </label>
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setReptileData({ ...reptileData, start_bid: e.target.value })}
                                    />
                                </div>
                                <div className="px-1 py-1">
                                    <p className="text-[9px] font-bold text-amber-500/80 italic leading-tight">
                                        * Belum termasuk ongkir & packing. Klik pilihan "Gratis" di bawah jika ingin menggratiskan.
                                    </p>
                                </div>
                            </div>
                            <div className="space-y-2">
                                <label className="text-xs font-black text-zinc-500 uppercase tracking-widest ml-1 flex items-center gap-1.5">
                                    <Plus size={12} className="text-amber-500" /> Kelipatan Bid <span className="text-red-500">*</span>
                                </label>
                                <div className="relative">
                                    <span className="absolute left-5 top-1/2 -translate-y-1/2 text-zinc-600 font-black text-xs">Rp</span>
                                    <input
                                        ...
                                        onChange={(e) => setReptileData({ ...reptileData, multiple: e.target.value })}
                                    />
                                </div>
                            </div>
                            <div className="space-y-2">
                                <label className="text-xs font-black text-zinc-500 uppercase tracking-widest ml-1 flex items-center gap-1.5">
                                    <Calendar size={12} className="text-amber-500" /> Tanggal Berakhir <span className="text-red-500">*</span>
                                </label>
                                <input
                                    type="datetime-local"
                                    required
                                    className="w-full bg-zinc-950 border border-zinc-800 text-white rounded-2xl px-5 py-4 focus:outline-none focus:border-amber-500 transition-all font-bold shadow-inner text-sm"
                                    value={reptileData.end_date}
                                    onChange={(e) => setReptileData({ ...reptileData, end_date: e.target.value })}
                                />
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label className="text-xs font-black text-zinc-500 uppercase tracking-widest ml-1">Jenis Kelamin <span className="text-red-500">*</span></label>
                                <select
                                    required
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-black text-zinc-500 uppercase tracking-widest ml-1">Jumlah Stok <span className="text-red-500">*</span></label>
                                <input
                                    type="number"
                                    required
                                    min="1"
                    ...
                    <div className="p-6 bg-zinc-950/50 border border-zinc-800 rounded-3xl space-y-6">
                        <div className="space-y-2">
                            <label className="text-xs font-black text-zinc-500 uppercase tracking-widest ml-1">Jenis Jangkauan Pengiriman <span className="text-red-500">*</span></label>
                            <select
                                required
                                className="w-full bg-zinc-950 border border-zinc-800 text-white rounded-2xl px-5 py-4 focus:outline-none focus:border-amber-500 transition-all font-bold appearance-none cursor-pointer shadow-inner text-sm"
                    ...
                    <div className="space-y-6">
                        <div className="space-y-2">
                            <label className="text-xs font-black text-zinc-500 uppercase tracking-widest ml-1">Deskripsi Produk <span className="text-red-500">*</span></label>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="p-6 bg-zinc-950 border border-zinc-800 rounded-2xl relative overflow-hidden group">
                                ...
                                <div className="text-sm text-zinc-400 leading-relaxed description-content relative z-10" dangerouslySetInnerHTML={{ __html: reptileData.shipping_description || '<span className="italic opacity-50">Mengambil data kebijakan dari profil toko Anda...</span>' }}></div>
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between ml-1">
                            <label className="text-xs font-black text-zinc-500 uppercase tracking-widest">Foto Produk (Maks 3) <span className="text-red-500">*</span></label>
                        </div>
                    ...
                    <div className="pt-8 border-t border-zinc-800">
                        <label className="flex items-start gap-4 cursor-pointer group mb-8 px-2">
                            ...
                            <span className="text-xs font-bold text-zinc-500 group-hover:text-zinc-300 transition-colors leading-relaxed">
                                Saya menyatakan bahwa data lelang dan ketentuan di atas adalah benar. Saya bertanggung jawab penuh atas proses lelang ini.
                            </span>
                        </label>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="bg-zinc-900 border border-zinc-800 rounded-[2.5rem] w-full max-w-md p-12 text-center relative z-10 shadow-[0_0_100px_rgba(245,158,11,0.1)] animate-in zoom-in-95 duration-300">
                        ...
                        <p className="text-zinc-400 mb-10 leading-relaxed font-medium">
                            Lelang Anda telah berhasil didaftarkan dan akan segera aktif setelah verifikasi sistem.
                        </p>
            ...
            <style jsx global>{`
                ...
                .quill-dark-editor .ql-snow .ql-picker-options { background-color: #09090b !important; border-color: #18181b !important; }`}</style>
        </div>
    );
}
