...
export default function LelangProdukPage() {
  ...
  useEffect(() => {
    if (reptileData.duration) {
      ...
      const end = new Date(start.getTime() + (parseInt(reptileData.duration, 10) || 1) * 24 * 60 * 60 * 1000);
  ...
  const handleAddAuction = async (e) => {
    ...
    try {
      // Check if product name is a protected species
      if (reptileData.name) {
      ...
      if (!reptileData.name || !reptileData.species || !reptileData.start_bid || !reptileData.multiple || !reptileData.duration || !cleanDescription || !reptileData.sex || !cleanShipping || !reptileData.shipping_type || reptileData.images.length === 0) {
        alert("Harap lengkapi semua data lelang, deskripsi, kebijakan pengiriman, jenis pengiriman, dan unggah minimal satu foto.");
        setIsSubmitting(false);
        return;
      }
      ...
      if (reptileData.bin_price) {
        const bin = parseInt(reptileData.bin_price.toString().replace(/\D/g, ""));
        const ob = parseInt(reptileData.start_bid.toString().replace(/\D/g, ""));
        if (bin <= ob) {
          alert("Harga Beli Sekarang (BIN) harus lebih tinggi dari Open Bid (OB).");
          setIsSubmitting(false);
          return;
        }
      }
    ...
    return (
      <div className="flex items-center justify-center min-h-[80vh] bg-zinc-950">
        <div className="relative">
          ...
          <Gavel className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-amber-500" size={24} />
        </div>
      </div>
    );
  }
  ...
  if (!hasShop) {
    return (
      <div className="max-w-4xl mx-auto py-20 text-center space-y-6">
        <div className="w-20 h-20 bg-zinc-900 border border-zinc-800 rounded-[10px] flex items-center justify-center mx-auto text-zinc-500">
          <AlertCircle size={40} />
        </div>
        <div>
          <h1 className="text-2xl font-black text-white">Toko Belum Terdaftar</h1>
          <p className="text-zinc-500 mt-2">Anda harus membuka toko terlebih dahulu sebelum dapat melakukan lelang.</p>
        </div>
        <Link href="/user/toko" className="inline-flex items-center gap-2 px-8 py-4 bg-amber-500 hover:bg-amber-400 text-zinc-950 font-black rounded-[10px] transition-all shadow-lg shadow-amber-500/20">
          Buka Toko Sekarang
  ...
  if (shopStatus?.toLowerCase() === "suspended") {
    return (
      <div className="max-w-4xl mx-auto py-20 text-center space-y-6">
        <div className="w-20 h-20 bg-red-500/10 border border-red-500/20 rounded-[10px] flex items-center justify-center mx-auto text-red-500 shadow-xl shadow-red-500/5">
          <AlertCircle size={40} />
        </div>
        <div>
          <h1 className="text-2xl font-black text-white">Toko Ditangguhkan</h1>
          <p className="text-zinc-500 mt-2">Maaf, toko Anda sedang dalam status penangguhan oleh Admin. Aktivitas lelang dinonaktifkan sementara.</p>
        </div>
        <button onClick={() => window.open("https://wa.me/628123456789", "_blank")} className="px-8 py-4 bg-white text-zinc-950 font-black rounded-[10px] hover:bg-zinc-200 transition-all active:scale-95">
          Hubungi Admin
        </button>
      </div>
    );
    ...
    <div className="max-w-5xl mx-auto py-10 space-y-10 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-3xl font-black text-white tracking-tight flex items-center gap-3">Mulai Lelang Baru</h1>
          <p className="text-zinc-500 mt-1 font-bold uppercase tracking-widest text-[10px] sm:text-xs">Buka lelang untuk produk terbaik Anda</p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          <Link href="/user/toko/daftar-produk" className="px-5 py-2.5 bg-zinc-900 hover:bg-zinc-800 text-white text-sm font-bold rounded-[10px] border border-zinc-800 transition-all flex items-center gap-2">
            Lihat Iklan Saya
          </Link>
        </div>
      </div>
      ...
      {quota && quota.remaining === 0 && (
        <div className="bg-red-500/10 border border-red-500/20 rounded-[10px] p-6 text-center space-y-3">
          ...
          <Link href="/user/toko/daftar-produk" className="inline-flex items-center gap-2 px-6 py-3 bg-zinc-800 hover:bg-zinc-700 text-white font-black rounded-[10px] transition-all">
            ...
            <form onSubmit={handleAddAuction} className="px-0 py-6 sm:p-8 space-y-8">
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full bg-zinc-950 border border-zinc-800 text-white rounded-[10px] px-5 py-4 focus:outline-none focus:border-amber-500 transition-all font-bold placeholder:text-zinc-700 shadow-inner"
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      required
                      className="w-full bg-zinc-950 border border-zinc-800 text-white rounded-[10px] px-5 py-4 focus:outline-none focus:border-amber-500 transition-all font-bold appearance-none cursor-pointer shadow-inner"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <input type="number" required min="1" className="w-full bg-zinc-950 border border-zinc-800 text-white rounded-[10px] px-5 py-4 focus:outline-none focus:border-amber-500 transition-all font-bold shadow-inner" value={reptileData.stock} onChange={(e) => setReptileData({ ...reptileData, stock: e.target.value })} />
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-xs font-black text-zinc-500 uppercase tracking-widest ml-1 flex items-center gap-1.5">
                      <Calendar size={12} className="text-white" /> Tanggal Mulai (Opsional / Sekarang)
                    </label>
                    <input
                      type="date"
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-black text-zinc-500 uppercase tracking-widest ml-1 flex items-center gap-1.5">
                      <Calendar size={12} className="text-white" /> Durasi Lelang <span className="text-red-500">*</span>
                    </label>
                    <select
                      required
                      className="w-full bg-zinc-950 border border-zinc-800 text-white rounded-[10px] px-5 py-4 focus:outline-none focus:border-amber-500 transition-all font-bold appearance-none cursor-pointer shadow-inner text-sm"
                ...
                {previewDates && (
                  <div className="bg-zinc-900/50 border border-zinc-800 p-6 rounded-[10px] flex flex-col md:flex-row gap-6 md:gap-12 text-sm mt-6 shadow-inner animate-in fade-in duration-300">
                    ...
                    <div>
                      <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-1.5">Berakhir:</p>
                      <p className="text-amber-500 font-bold">{previewDates.end}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-1.5">Durasi:</p>
                      <p className="text-white font-bold">{previewDates.duration}</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="p-4 sm:p-6 bg-zinc-950/50 border border-zinc-800 rounded-[10px] space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4 border-t border-zinc-800/50">
                  <div className="space-y-2">
                    <label className="text-xs font-black text-zinc-500 uppercase tracking-widest ml-1 flex items-center gap-1.5">
                      <Coins size={12} className="text-white" /> Open Bid (OB) <span className="text-red-500">*</span>
                    </label>
                    <div className="relative">
                      <span className="absolute left-5 top-1/2 -translate-y-1/2 text-zinc-600 font-black text-xs">Rp</span>
                      <input
                        type="text"
                        inputMode="numeric"
                        required
                    ...
                    <div className="px-1 py-1">
                      <p className="text-[9px] font-bold text-amber-500/80 italic leading-tight">&quot;Belum termasuk ongkir & packing. Klik pilihan &quot;Gratis&quot; di bawah jika ingin menggratiskan. Semua Ongkir & Packing, ditanggung pembeli</p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <label className="text-xs font-black text-zinc-500 uppercase tracking-widest ml-1 flex items-center gap-1.5">
                      <Plus size={12} className="text-white" /> Kelipatan Bid <span className="text-red-500">*</span>
                    </label>
                    <div className="relative">
                      <span className="absolute left-5 top-1/2 -translate-y-1/2 text-zinc-600 font-black text-xs">Rp</span>
                      <input
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-black text-zinc-500 uppercase tracking-widest ml-1 flex items-center gap-1.5">
                      <Tag size={12} className="text-white" /> Beli Sekarang / BIN (Opsional)
                    </label>
                    <div className="relative">
                      <span className="absolute left-5 top-1/2 -translate-y-1/2 text-zinc-600 font-black text-xs">Rp</span>
                      <input
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <label className="flex items-center gap-4 cursor-pointer group bg-zinc-900/50 p-4 rounded-[10px] border border-zinc-800/50 hover:border-amber-500/30 transition-all">
                      ...
                      <span className="text-xs font-bold text-zinc-400 group-hover:text-zinc-200 transition-colors">Bebas Biaya Packing (Gratis)</span>
                    </label>
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="text-xs font-black text-zinc-500 uppercase tracking-widest ml-1">
                    Jenis Jangkauan Pengiriman <span className="text-red-500">*</span>
                  </label>
                  <select
                    required
                    className="w-full bg-zinc-950 border border-zinc-800 text-white rounded-[10px] px-5 py-4 focus:outline-none focus:border-amber-500 transition-all font-bold appearance-none cursor-pointer shadow-inner text-sm"
                  ...
                  <div className="p-4 sm:p-6 bg-zinc-950 border border-zinc-800 rounded-[10px] relative overflow-hidden group">
                    ...
                    <div
                      ...
                      dangerouslySetInnerHTML={{
                        __html: reptileData.shipping_description || '<span className="italic opacity-50">Mengambil data kebijakan dari profil toko Anda...</span>',
                      }}
                    ></div>
                    <div className="mt-4 pt-4 border-t border-zinc-800/50 flex items-center justify-between relative z-10">
                      ...
                      <Link href="/user/toko/edit-toko" className="text-[10px] font-black text-amber-500 hover:text-amber-400 uppercase tracking-widest transition-colors flex items-center gap-1">
                ...
                <div className="space-y-3 bg-zinc-900/40 p-5 border border-zinc-800 rounded-[10px]">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <button
                      ...
                      className={`py-3.5 rounded-[10px] font-bold text-xs uppercase tracking-wider transition-all border ${mediaType === "image" ? "bg-amber-500 border-amber-500 text-zinc-950 font-black shadow-md shadow-amber-500/10" : "bg-zinc-950 border-zinc-800 text-zinc-400 hover:text-white"}`}
                    >
                      Gambar / Foto
                    </button>
                    <button
                      ...
                      className={`py-3.5 rounded-[10px] font-bold text-xs uppercase tracking-wider transition-all border ${mediaType === "video" ? "bg-amber-500 border-amber-500 text-zinc-950 font-black shadow-md shadow-amber-500/10" : "bg-zinc-950 border-zinc-800 text-zinc-400 hover:text-white"}`}
                    >
                      Video
                    </button>
                  </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {reptileData.images.map((img, index) => (
                    <div key={index} className="relative aspect-square rounded-[10px] overflow-hidden group border border-zinc-800 shadow-2xl flex items-center justify-center bg-zinc-900">
              ...
              <div className="pt-8 border-t border-zinc-800">
                <label className="flex items-start gap-4 cursor-pointer group mb-8 px-2">
                  ...
                  <span className="text-xs font-bold text-zinc-500 group-hover:text-zinc-300 transition-colors leading-relaxed">Saya menyatakan bahwa data lelang and ketentuan di atas adalah benar. Saya bertanggung jawab penuh atas proses lelang ini.</span>
                </label>

                <button
                  type="submit"
                  disabled={isSubmitting || isUploading || reptileData.images.length === 0}
                  className="w-full py-5 rounded-[10px] bg-amber-500 hover:bg-amber-400 text-zinc-950 font-black shadow-2xl shadow-amber-500/20 transition-all flex items-center justify-center gap-3 group active:scale-[0.98] disabled:opacity-50 disabled:grayscale disabled:cursor-not-allowed"
                >
                  {isSubmitting ? (
          ...
          {showSuccessModal && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-[10px] w-full max-w-md p-12 text-center relative z-10 shadow-[0_0_100px_rgba(245,158,11,0.1)] animate-in zoom-in-95 duration-300">
                ...
                <h3 className="text-3xl font-black text-white mb-4">Lelang Dimulai!</h3>
                <p className="text-zinc-400 mb-10 leading-relaxed font-medium">Lelang Anda telah berhasil didaftarkan dan akan segera aktif setelah verifikasi sistem.</p>
                <div className="grid grid-cols-1 gap-4">
                  {createdListingId && (
                    <Link href={`/toko/detail-lelang/${createdListingId}`} className="w-full bg-amber-500 hover:bg-amber-400 text-zinc-950 font-black py-4 rounded-[10px] transition-all shadow-xl flex items-center justify-center gap-2 active:scale-95">
                      <Gavel size={20} />
                      Buka Detail Lelang
                    </Link>
                  )}
            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-[10px] w-full max-w-md p-12 text-center relative z-10 shadow-2xl animate-in zoom-in-95 duration-300">
                ...
                <button
                  ...
                  className="w-full bg-red-500 hover:bg-red-400 text-zinc-950 font-black py-4 rounded-[10px] transition-all shadow-xl shadow-red-500/20"
                >
                  Saya Mengerti
                </button>
              </div>
