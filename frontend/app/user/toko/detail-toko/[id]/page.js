      ...
      <div className="max-w-7xl mx-auto space-y-6 sm:space-y-10 animate-in fade-in duration-500">
        ...
        {pendingRequest && (
          <div className="p-6 bg-amber-500/10 border border-amber-500/20 rounded-[10px] flex flex-col md:flex-row items-start md:items-center justify-between gap-6 hover:bg-amber-500/15 transition-all">
            ...
            <Link href="/user/toko/upgrade-toko" className="px-5 py-2.5 bg-zinc-900 border border-zinc-800 hover:border-zinc-700 text-amber-500 hover:bg-amber-500/10 rounded-[10px] text-xs font-black uppercase tracking-widest transition-all shrink-0 no-underline">
              Lihat Detail
            </Link>
          </div>
        )}
        ...
        {/* Shop Details Card */}
        <div className="bg-zinc-900 border border-zinc-800 rounded-[10px] p-4 sm:p-8 relative overflow-hidden">
          ...
          <div className="flex flex-col xs:flex-row items-center gap-4 sm:gap-8 relative z-10">
            <div className="w-16 h-16 sm:w-24 sm:h-24 bg-zinc-950 rounded-[10px] overflow-hidden border border-zinc-800 shrink-0 relative group">
              {shopData.logo_url ? (
            ...
            {/* Quota & membership status code elements */}
            <div className="flex flex-wrap sm:flex-col items-center sm:items-start justify-center gap-3 sm:gap-2 border-t sm:border-t-0 sm:border-l border-zinc-850 pt-4 sm:pt-0 sm:pl-8 shrink-0 w-full xs:w-auto">
              <div className="group/code flex items-center gap-3 bg-zinc-950/50 border border-zinc-800/80 hover:border-emerald-500/40 px-4 py-2 rounded-[10px] transition-all duration-500 backdrop-blur-md">
          ...
          {/* Quick info row (Status & Action buttons) */}
          <div className="mt-8 pt-6 border-t border-zinc-800/80 flex flex-col sm:flex-row items-center justify-between gap-6 relative z-10">
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-3">
              <div className="flex items-center gap-2 font-bold bg-zinc-950/30 px-4 py-1.5 rounded-[10px] border border-zinc-800/50 text-[10px] sm:text-xs">
                <MapPin size={14} className="text-zinc-500 shrink-0" />
                <span className="text-[10px] sm:text-xs font-bold text-zinc-400 truncate max-w-[150px] uppercase">{shopData.city || "-"}</span>
              </div>

              {shopData.status === "pending" && (
                <div className="flex items-center gap-2 font-bold text-amber-500 bg-amber-500/10 px-4 py-1.5 rounded-[10px] border border-amber-500/20 text-[10px] sm:text-xs">
                  <span className="w-1.5 h-1.5 bg-amber-500 rounded-full animate-pulse"></span>
                  PENDING VERIFIKASI ADMIN
                </div>
              )}

              {shopData.status === "rejected" && <div className="flex items-center gap-2 font-bold text-orange-500 bg-orange-500/10 px-4 py-1.5 rounded-[10px] border border-orange-500/20 text-[10px] sm:text-xs">DITOLAK ADMIN</div>}

              {shopData.status === "inactive" && <div className="flex items-center gap-2 font-bold text-red-500 bg-red-500/10 px-4 py-1.5 rounded-[10px] border border-red-500/20 text-[10px] sm:text-xs">NONAKTIF</div>}

              {shopData.status === "active" && (
                <div className="flex items-center gap-2 font-bold text-emerald-500 bg-emerald-500/10 px-4 py-1.5 rounded-[10px] border border-emerald-500/20 text-[10px] sm:text-xs">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
              {currentUser && currentUser.id === shopData.user_id && (
                <>
                  {isUpgradeLocked ? (
                    <button disabled className="w-full sm:w-auto px-5 py-3 sm:py-2.5 bg-zinc-800 text-zinc-500 text-xs sm:text-sm font-bold rounded-[10px] transition-all flex items-center justify-center gap-2 cursor-not-allowed opacity-50 text-center" title="Fitur Upgrade Toko dinonaktifkan sementara oleh Admin">
              ...
              {currentUser && currentUser.id === shopData.user_id && (
                <Link href="/user/toko/edit-toko" className="w-full sm:w-auto px-5 py-3 sm:py-2.5 bg-zinc-800 hover:bg-zinc-700 text-white text-xs sm:text-sm font-bold rounded-[10px] transition-all flex items-center justify-center gap-2 text-center">
                  <PencilLine size={16} /> Edit Profile Toko
                </Link>
              )}
              <Link href="/user/toko/dashboard" className="w-full sm:w-auto px-5 py-3 sm:py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white text-xs sm:text-sm font-black rounded-[10px] transition-all flex items-center justify-center gap-2 text-center">
                Dashboard
              </Link>
            </div>
          </div>
          ...
          <div className="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-zinc-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-6">
              ...
              <div className="flex flex-wrap gap-2 sm:gap-4">
                {userBankAccounts && userBankAccounts.length > 0 ? (
                  userBankAccounts.map((bank, idx) => (
                    <div key={idx} className="bg-zinc-950/50 border border-zinc-800 px-3 sm:px-4 py-2 sm:py-3 rounded-[10px] flex items-center gap-2 sm:gap-3">
                      <div className="w-7 h-7 sm:w-8 sm:h-8 bg-zinc-800 rounded-[10px] flex items-center justify-center text-[9px] sm:text-[10px] font-black text-emerald-500 uppercase shrink-0">{(bank.bank_name || bank.bankName || "").substring(0, 3)}</div>
                      <div className="min-w-0">
            ...
            <div className="bg-zinc-950/50 border border-zinc-800 px-4 sm:px-6 py-2 sm:py-3 rounded-[10px] shrink-0 w-full sm:w-auto text-left">
        ...
        {shopData.status?.toLowerCase() === "suspended" && (
          <div className="bg-red-500/10 border border-red-500/20 rounded-[10px] p-8 flex flex-col md:flex-row items-center gap-6 animate-in slide-in-from-top-4 duration-500">
            <div className="w-16 h-16 bg-red-500 text-white rounded-[10px] flex items-center justify-center shrink-0">
              <AlertCircle size={32} />
            </div>
            ...
            <button onClick={() => window.open("https://wa.me/628123456789", "_blank")} className="w-full md:w-auto px-6 py-3 bg-white text-zinc-950 font-black rounded-[10px] hover:bg-zinc-200 transition-all active:scale-95 text-sm text-center">
              Hubungi Admin
            </button>
          </div>
        )}
        ...
        {shopData.status?.toLowerCase() === "rejected" && (
          <div className="bg-orange-500/10 border border-orange-500/20 rounded-[10px] p-8 flex flex-col md:flex-row items-center gap-6 animate-in slide-in-from-top-4 duration-500">
            <div className="w-16 h-16 bg-orange-500 text-white rounded-[10px] flex items-center justify-center shrink-0">
              <XCircle size={32} />
            </div>
            <div className="flex-1 text-center md:text-left">
              <h3 className="text-xl font-black text-white">Verifikasi Toko Ditolak</h3>
              <div className="mt-2 p-4 bg-zinc-950/50 border border-zinc-800 rounded-[10px]">
            ...
            <div className="flex flex-col sm:flex-row md:flex-col gap-3 w-full md:w-auto">
              <Link href="/user/toko/edit-toko" className="w-full md:w-auto px-6 py-3 bg-white text-zinc-950 font-black rounded-[10px] hover:bg-zinc-200 transition-all active:scale-95 text-sm text-center whitespace-nowrap font-bold">
                Perbaiki Profil
              </Link>
              <button onClick={handleResubmit} disabled={isSubmitting} className="w-full md:w-auto px-6 py-3 bg-orange-500 text-white font-black rounded-[10px] hover:bg-orange-400 transition-all active:scale-95 text-sm text-center whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          {showRules && (
            <div className="px-8 pb-8 animate-in slide-in-from-top-2 duration-300">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8 text-sm text-zinc-300">
                ...
                <div className="space-y-8">
                  <div className="space-y-3">
                    <h3 className="font-black text-white flex items-center gap-2 text-base">
                      <span className="w-7 h-7 bg-amber-500 text-zinc-950 rounded-[10px] flex items-center justify-center text-xs">1</span>
                  ...
                  <div className="space-y-3">
                    <h3 className="font-black text-white flex items-center gap-2 text-base">
                      <span className="w-7 h-7 bg-amber-500 text-zinc-950 rounded-[10px] flex items-center justify-center text-xs">2</span>
                      Kondisi Hewan
                    </h3>
                    <ul className="list-disc list-outside ml-9 text-zinc-400 space-y-2 leading-relaxed">
                  ...
                  <div className="space-y-3">
                    <h3 className="font-black text-white flex items-center gap-2 text-base">
                      <span className="w-7 h-7 bg-amber-500 text-zinc-950 rounded-[10px] flex items-center justify-center text-xs">3</span>
                      Foto Produk
                    </h3>
                    <ul className="list-disc list-outside ml-9 text-zinc-400 space-y-2 leading-relaxed">
                  ...
                  <div className="space-y-3">
                    <h3 className="font-black text-white flex items-center gap-2 text-base">
                      <span className="w-7 h-7 bg-amber-500 text-zinc-950 rounded-[10px] flex items-center justify-center text-xs">4</span>
                      Penetapan Harga
                    </h3>
                    <ul className="list-disc list-outside ml-9 text-zinc-400 space-y-2 leading-relaxed">
                ...
                {/* Kolom 2 */}
                <div className="space-y-8">
                  <div className="bg-amber-500/5 border border-amber-500/20 p-5 rounded-[10px] space-y-3">
                  ...
                  <div className="space-y-3">
                    <h3 className="font-black text-white flex items-center gap-2 text-base">
                      <span className="w-7 h-7 bg-amber-500 text-zinc-950 rounded-[10px] flex items-center justify-center text-xs">6</span>
                  ...
                  <div className="space-y-3">
                    <h3 className="font-black text-white flex items-center gap-2 text-base">
                      <span className="w-7 h-7 bg-amber-500 text-zinc-950 rounded-[10px] flex items-center justify-center text-xs">7</span>
                      Larangan Penjualan
                    </h3>
                    <ul className="list-disc list-outside ml-9 text-zinc-400 space-y-2 leading-relaxed">
                  ...
                  <div className="space-y-3">
                    <h3 className="font-black text-white flex items-center gap-2 text-base">
                      <span className="w-7 h-7 bg-amber-500 text-zinc-950 rounded-[10px] flex items-center justify-center text-xs">8</span>
                  ...
                  <div className="space-y-3">
                    <h3 className="font-black text-white flex items-center gap-2 text-base">
                      <span className="w-7 h-7 bg-amber-500 text-zinc-950 rounded-[10px] flex items-center justify-center text-xs">9</span>
                      Sanksi
                    </h3>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-[10px] p-6 sm:p-10 space-y-6 sm:space-y-8">
          ...
          {loadingReviews ? (
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                {currentReviews.map((rev) => (
                  <div key={rev.id} className="bg-zinc-950/40 border border-zinc-800/80 rounded-[10px] p-5 sm:p-6 space-y-4 hover:border-zinc-750 transition-all flex flex-col justify-between group">
                    <div className="space-y-3">
                      <div className="flex justify-between items-start gap-4">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="w-10 h-10 rounded-[10px] bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-500 font-black text-sm shrink-0 uppercase">{(rev.user?.name || rev.user?.username || "B").substring(0, 2)}</div>
                          <div className="min-w-0">
                        ...
                        <div className="bg-zinc-900/80 px-2.5 py-1.5 rounded-[10px] border border-zinc-800/60 shrink-0">{renderStars(rev.rating)}</div>
                      </div>

                      <div className="bg-zinc-900/30 border border-zinc-850 p-3.5 rounded-[10px] text-xs text-zinc-400 font-medium leading-relaxed italic break-words">&ldquo;{rev.review || "Pembeli memberikan rating bintang 5 otomatis tanpa komentar tertulis."}&rdquo;</div>
                    </div>

                    {rev.product && (
                      <div className="pt-3 border-t border-zinc-900 flex items-center gap-3 bg-zinc-900/10 p-2.5 rounded-[10px] border border-zinc-850 mt-2 shrink-0">
                        <div className="w-10 h-10 rounded-[10px] overflow-hidden bg-zinc-950 border border-zinc-800 shrink-0 relative">
                          {(() => {
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-zinc-800/80 pt-6 mt-6">
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      className={`w-8 h-8 rounded-[10px] flex items-center justify-center border text-xs font-black transition-all active:scale-95 ${currentPage === 1 ? "border-zinc-850 bg-zinc-900/10 text-zinc-600 cursor-not-allowed" : "border-zinc-800 bg-zinc-900 hover:bg-zinc-850 text-zinc-300 hover:text-white"}`}
                    >
                      <ChevronLeft size={16} />
                    </button>
                      ...
                      if (
                        ...
                        return (
                          <button
                            ...
                            className={`w-8 h-8 rounded-[10px] text-xs font-black transition-all active:scale-95 flex items-center justify-center border ${currentPage === pageNumber ? "bg-emerald-500 border-emerald-400 text-zinc-950" : "bg-zinc-900 border-zinc-850 text-zinc-400 hover:bg-zinc-850 hover:text-zinc-200"}`}
                          >
                            {pageNumber}
                          </button>
                        );
                    ...
                    <button
                      ...
                      className={`w-8 h-8 rounded-[10px] flex items-center justify-center border text-xs font-black transition-all active:scale-95 ${currentPage === totalPages ? "border-zinc-850 bg-zinc-900/10 text-zinc-600 cursor-not-allowed" : "border-zinc-850 bg-zinc-900 hover:bg-zinc-850 text-zinc-300 hover:text-white"}`}
                    >
            ...
            <div className="py-16 text-center space-y-4 bg-zinc-950/20 border border-zinc-800 rounded-[10px] border-dashed">
              <div className="w-16 h-16 bg-zinc-900 rounded-[10px] flex items-center justify-center text-zinc-700 mx-auto">
                <Star size={28} />
              </div>
              <div className="space-y-1">
