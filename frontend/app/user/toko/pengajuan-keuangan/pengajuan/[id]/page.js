...
import { ChevronLeft, ArrowUpRight, CheckCircle2, CreditCard, Info, AlertCircle, DollarSign, Package, MapPin, Calendar, Send } from "lucide-react";
...
export default function PengajuanPencairanPage({ params }) {
  ...
  const totalDana = getOrderSubtotal(order) + Number(order.shipping_cost || 0) + Number(order.packing_cost || 0);
                    ...
                    {order.items && order.items.length > 0 ? (
                        ...
                        <div key={item.id || idx} className="flex items-center gap-4 border-b border-zinc-800/40 pb-3 last:border-0 last:pb-0">
                          <div className="w-16 h-16 bg-zinc-900 rounded-[10px] overflow-hidden border border-zinc-800 shrink-0">
                            {item.product?.images && isVideoUrl(item.product.images) ? (
                              ...
                              <img src={getImageUrl(item.product?.images) || "https://placehold.co/100x100?text=No+Image"} className="w-full h-full object-cover" />
                            )}
                          </div>
                          <div className="min-w-0 flex-1">
                            <h3 className="text-white font-black text-sm line-clamp-1">{item.product?.name || "Produk"}</h3>
                            <p className="text-xs text-zinc-500 font-bold mt-1 italic">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="min-w-0">
                          <h3 className="text-white font-black text-sm line-clamp-1">{order.product?.name || "Produk dihapus"}</h3>
                          <p className="text-xs text-zinc-500 font-bold mt-1 italic">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-[10px] p-8 space-y-8">
            <div className="space-y-6">
              ...
              <div className="bg-zinc-950/50 rounded-[10px] p-6 border border-zinc-800/50 space-y-6">
                ...
                <div className="pt-4 border-t border-zinc-800/50">
                  <p className="text-[10px] text-zinc-500 font-medium leading-relaxed italic">Admin akan mengirimkan dana ke rekening yang tertera di atas. Pastikan data sudah benar.</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-amber-500/5 border border-amber-500/20 rounded-[10px] p-6 flex gap-4 items-start">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-amber-500/70 font-medium leading-relaxed">Pengajuan ini hanya bersifat notifikasi ke Admin. Admin akan segera memproses transfer bukti pembayaran.</p>
              </div>
            </div>
          </div>
        </div>
