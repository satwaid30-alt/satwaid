...
export default function RiwayatTransaksiSeller() {
    ...
    const fetchShopOrders = async () => {
        ...
        try {
            ...
            // Get Shop ID first
            const shopRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/shops/user/${user.id}`);
            const shopData = await shopRes.json();

            if (shopRes.ok && shopData.data) {
                const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/orders/shop/${shopData.data.id}`);
    ...
    const totalEarnings = orders.filter(o =>
    ...
    const totalDisbursedAmount = orders.filter(o => o.disbursed_at || o.disbursement_proof).reduce((acc, curr) => {
        const total = (Number(curr.price || 0) * Number(curr.quantity || 1)) +
            Number(curr.shipping_cost || 0) +
            Number(curr.packing_cost || 0) -
            (Number(curr.additional_fee || 0));
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl font-black text-white tracking-tight mb-2 uppercase italic">Pengajuan Keuangan</h1>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-2xl md:rounded-[2.5rem] p-4 md:p-8 space-y-2 md:space-y-4 relative overflow-hidden group">
                    ...
                    <div>
                        <p className="text-[8px] md:text-xs font-black text-zinc-500 uppercase tracking-widest">Sudah Dicairkan</p>
                        <h3 className="text-xs sm:text-sm md:text-2xl font-black text-emerald-500 mt-0.5 md:mt-1 leading-none">
                            {formatPrice(totalDisbursedAmount)}
                        </h3>
                    </div>
                </div>
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-[2rem] p-3 flex flex-col md:flex-row gap-4">
                ...
                <div className="flex items-center gap-2 bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-2 min-w-[200px]">
                    ...
                    >
                        ...
                        <option value="disbursed" className="bg-zinc-900 text-white">Sudah Cair</option>
                        <option value="requested" className="bg-zinc-900 text-white">Pengajuan (Menunggu Konfirmasi)</option>
                        <option value="pending" className="bg-zinc-900 text-white">Belum Diajukan</option>
                    </select>
                </div>
            </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-zinc-950/50 border-b border-zinc-800">
                                ...
                                <th className="px-8 py-5 text-[10px] font-black text-zinc-500 uppercase tracking-widest text-right">Pencairan</th>
                            ...
                            {isLoading ? (
                                [1, 2, 3, 4, 5].map(i => (
                                    <tr key={i} className="animate-pulse">
                                        <td colSpan={9} className="px-8 py-10 h-24 bg-zinc-900/50"></td>
                                    </tr>
                                ))
                                    ...
                                    <tr key={order.id} className="hover:bg-zinc-800/20 transition-colors group border-b border-zinc-800/50">
                                        ...
                                        <td className="px-8 py-6 text-center">
                                            {order.disbursed_at ? (
                                                ...
                                                <span className="text-[10px] font-bold text-zinc-600 italic">Belum Cair</span>
                                            )}
                                        </td>
                                        <td className="px-8 py-6">
                                            <div className="space-y-1">
                                                <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-tighter">
                                                    Produk: {formatPrice(Number(order.price) * Number(order.quantity))}
                                                </p>
                                                <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-tighter">
                                                    Ongkir + Pack: {formatPrice(Number(order.shipping_cost) + Number(order.packing_cost))}
                                                </p>
                                            </div>
                                        </td>
                                        <td className="px-8 py-6 text-right">
                                            <div className="flex flex-col items-end">
                                                ...
                                                {order.additional_fee > 0 && (
                                                    <span className="text-[9px] font-bold text-red-500/70 italic">Potongan: {formatPrice(order.additional_fee)}</span>
                                                )}
                                            </div>
                                        </td>
                                        <td className="px-8 py-6 text-center">
                                        ...
                                        <td className="px-8 py-6 text-center">
                                            <div className="flex justify-center items-center gap-2">
                                                {(order.disbursed_at || order.disbursement_proof) ? (
                                                    <Link
                                                        href={`/user/toko/pengajuan-keuangan/detail/${order.id}`}
                                                        className="flex items-center gap-2 px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border border-zinc-700 hover:border-zinc-600 group/btn shadow-inner min-w-[140px] justify-center"
                                                    >
                                                    ...
                                                    <Link
                                                        href={`/user/toko/pengajuan-keuangan/pengajuan/${order.id}`}
                                                        className="px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-zinc-950 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-emerald-500/20 active:scale-[0.98] inline-flex items-center gap-2"
                                                    >
            ...
            <div className="md:hidden space-y-4">
                {isLoading ? (
                    [1, 2, 3].map(i => (
                        <div key={i} className="bg-zinc-900 border border-zinc-800 rounded-3xl p-6 h-48 animate-pulse"></div>
                    ))
                ) : currentItems.length > 0 ? (
                    currentItems.map((order, index) => (
                        <div key={order.id} className="bg-zinc-900 border border-zinc-800 rounded-3xl p-5 space-y-4 shadow-xl active:scale-[0.99] transition-all duration-300">
                            ...
                            <div className="flex items-center justify-between border-b border-zinc-800/60 pb-3">
                                ...
                                <div>
                                    {(order.disbursed_at || order.disbursement_proof) ? (
                                        <span className="px-2 py-1 bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 rounded-lg text-[8px] font-black uppercase tracking-widest">Cair</span>
                                    ) : order.status === 'disbursement_requested' ? (
                                        ...
                                        <span className="px-2 py-1 bg-zinc-800 text-zinc-400 border border-zinc-700 rounded-lg text-[8px] font-black uppercase tracking-widest">Blm Diajukan</span>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div className="min-w-0 flex-1 space-y-1">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className="px-1.5 py-0.5 bg-zinc-950 text-[8px] text-zinc-500 rounded font-black uppercase tracking-widest border border-zinc-800">
                                            ID: {order.product?.product_id || "-"}
                                        </span>
                            ...
                            <div className="bg-zinc-950/40 rounded-2xl p-4 border border-zinc-800/40 space-y-3">
                                ...
                                <div className="flex justify-between items-center text-[10px]">
                                    ...
                                    {order.disbursement_requested_at ? (
                                        ...
                                        <span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 rounded-md text-[8px] font-black uppercase tracking-widest">
                                            Langsung Pencairan
                                        </span>
                                    ) : (
                                ...
                                <div className="flex justify-between items-center text-[10px]">
                                    <span className="text-zinc-500 font-bold uppercase tracking-widest">Tanggal Pencairan</span>
                                    {order.disbursed_at ? (
                                ...
                                <div className="border-t border-zinc-800/40 pt-2.5 flex justify-between items-center text-[10px]">
                                    ...
                                    <div className="text-right text-[9px] text-zinc-400 space-y-0.5 font-bold">
                                        ...
                                        {order.additional_fee > 0 && (
                                            <p className="text-red-500/70 font-bold">Potongan: {formatPrice(order.additional_fee)}</p>
                                        )}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between gap-4 pt-1">
                                ...
                                <div className="flex-1 max-w-[180px]">
                                    {(order.disbursed_at || order.disbursement_proof) ? (
                                        <Link
                                            href={`/user/toko/pengajuan-keuangan/detail/${order.id}`}
                                            className="w-full flex items-center justify-center gap-1.5 py-3 bg-zinc-800 hover:bg-zinc-700 text-white rounded-xl text-[9px] font-black uppercase tracking-widest transition-all border border-zinc-700 shadow-md shadow-zinc-950/20"
                                        >
                                            <FileText size={12} className="text-emerald-500" />
                                            Detail
                                        </Link>
                                    ) : order.status === 'disbursement_requested' ? (
                                        <div className="w-full flex items-center justify-center gap-1.5 py-3 bg-blue-500/10 text-blue-400 rounded-xl text-[9px] font-black uppercase tracking-widest border border-blue-500/20 cursor-default text-center leading-tight">
                                            Proses Review
                                        </div>
                                    ) : (
                                        <Link
                                            href={`/user/toko/pengajuan-keuangan/pengajuan/${order.id}`}
                                            className="w-full flex items-center justify-center gap-1.5 py-3 bg-emerald-500 hover:bg-emerald-400 text-zinc-950 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all shadow-lg shadow-emerald-500/20 active:scale-[0.98]"
                                        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-10">
                <div className="p-8 bg-zinc-900 border border-zinc-800 rounded-[2.5rem] flex gap-6 items-start group hover:border-emerald-500/30 transition-all">
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-xs text-zinc-500 leading-relaxed font-medium">
                            Dana akan diproses oleh Admin segera setelah pembeli mengonfirmasi pesanan diterima atau saat transaksi dinyatakan selesai. Proses transfer biasanya memakan waktu 1-3 hari kerja.
                        </p>
                    </div>
                </div>
                <div className="p-8 bg-zinc-900 border border-zinc-800 rounded-[2.5rem] flex gap-6 items-start group hover:border-blue-500/30 transition-all">
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-xs text-zinc-500 leading-relaxed font-medium">
                            Pencairan dana yang Anda terima adalah nilai bersih (Subtotal Produk + Ongkir + Packing) dikurangi biaya tambahan jika ada (misal: biaya admin transfer antar bank).
                        </p>
                    </div>
                </div>
            </div>
