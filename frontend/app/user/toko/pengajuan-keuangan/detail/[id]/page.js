...
import { ChevronLeft, Receipt, FileText, ArrowUpRight, Calendar, ShoppingBag, CreditCard, DollarSign, Info, CheckCircle2 } from "lucide-react";
...
export default function DetailPencairanPage({ params }) {
  ...
  const cleanTotal = getOrderSubtotal(order) + Number(order.shipping_cost) + Number(order.packing_cost) - (Number(order.additional_fee) || 0);

  return (
    <div className="max-w-5xl mx-auto px-4 md:px-0 space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      {/* Header Navigation */}
        ...
        <div className="flex items-center gap-3">
          <div className="px-5 py-3 bg-zinc-900 border border-zinc-800 rounded-2xl flex flex-col items-end w-full sm:w-auto">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs sm:text-sm font-black text-emerald-500 uppercase italic">Dana Berhasil Ditransfer</span>
            </div>
          </div>
        </div>
      </div>
          ...
          {order.disbursement_notes && (
            <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-5 md:p-8 flex gap-4 md:gap-6 items-start">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-zinc-300 font-medium italic leading-relaxed">{order.disbursement_notes}</p>
              </div>
            </div>
          )}
        </div>
            ...
            <div className="relative z-10 space-y-8">
              ...
              <div className="space-y-4">
                <div className="p-4 md:p-6 bg-zinc-950/50 rounded-3xl border border-zinc-800/50 space-y-4">
                  ...
                  <div className="flex justify-between items-center text-xs">
                    ...
                    <span className={Number(order.shipping_cost) === 0 ? "text-emerald-500 font-black italic" : "text-white font-black"}>{Number(order.shipping_cost) === 0 ? "Gratis" : formatPrice(order.shipping_cost)}</span>
                  </div>
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-zinc-500 font-bold uppercase tracking-widest">Biaya Packing</span>
                    <span className={Number(order.packing_cost) === 0 ? "text-emerald-500 font-black italic" : "text-white font-black"}>{Number(order.packing_cost) === 0 ? "Gratis" : formatPrice(order.packing_cost)}</span>
                  </div>
                ...
                <div className="bg-zinc-950/30 rounded-3xl p-4 md:p-6 border border-zinc-800/50 space-y-4">
                  ...
                  <div className="flex justify-between items-end">
                    <div>
                      ...
                      <p className="text-xs font-black text-zinc-400 uppercase leading-none">
              ...
              <div className="flex items-center gap-4 text-zinc-500">
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-zinc-300">{order.disbursed_at ? new Date(order.disbursed_at).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" }) : "-"}</p>
                </div>
              </div>
            </div>
      ...
      {/* Image Zoom Overlay */}
      {showZoom && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-10 animate-in fade-in duration-300" onClick={() => setShowZoom(false)}>
