...
export default function TransactionCompletePage({ params }) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (tabParam && ["complete", "complain"].includes(tabParam)) {
  ...
  const handleImageChange = (e) => {
    ...
    if (hasBlockedExtension) {
      setModalConfig({
        ...
        message: "Hanya gambar (JPG, PNG, WEBP, GIF) yang diperbolehkan.",
        type: "error",
      });
      setShowSuccessModal(true);
      e.target.value = "";
    ...
    if (!allowedMimeTypes.includes(file.type)) {
      setModalConfig({
        ...
        message: "Gunakan format JPG, PNG, WEBP, atau GIF.",
        type: "error",
      });
      setShowSuccessModal(true);
      e.target.value = "";
    ...
    // --- Validasi 4: Ukuran file maksimal 1 MB ---
    if (file.size > 1 * 1024 * 1024) {
      setModalConfig({
        title: "Ukuran Terlalu Besar",
        message: "Ukuran foto maksimal adalah 1 MB.",
        type: "error",
      });
      setShowSuccessModal(true);
      e.target.value = "";
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 px-4">
      ...
      <div className="flex items-center justify-between">
        <Link href={`/user/pesanan/transaksi/${id}`} replace className="flex items-center gap-2 text-zinc-400 hover:text-white transition-colors group">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-3xl md:rounded-[3rem] overflow-hidden">
        ...
        <div className="flex border-b border-zinc-800 p-1.5 md:p-2 gap-1.5 md:gap-2">
          <button onClick={() => setActiveTab("complete")} className={`flex-1 py-4 md:py-6 rounded-2xl md:rounded-[2rem] flex flex-col md:flex-row items-center justify-center gap-1.5 md:gap-3 transition-all ${activeTab === "complete" ? "bg-blue-500 text-zinc-950 font-black" : "text-zinc-500 hover:text-white font-bold"}`}>
            <CheckCircle2 size={18} className="md:w-5 md:h-5" />
            <span className="text-[10px] md:text-sm uppercase tracking-widest">Diterima</span>
          </button>
          <button onClick={() => setActiveTab("complain")} className={`flex-1 py-4 md:py-6 rounded-2xl md:rounded-[2rem] flex flex-col md:flex-row items-center justify-center gap-1.5 md:gap-3 transition-all ${activeTab === "complain" ? "bg-red-500 text-white font-black" : "text-zinc-500 hover:text-white font-bold"}`}>
            ...
            <form onSubmit={handleSubmitComplete} className="space-y-10 animate-in fade-in slide-in-from-left-4 duration-500">
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-center gap-3 md:gap-4 py-6 md:py-8 bg-zinc-950/50 rounded-2xl md:rounded-[2.5rem] border border-zinc-800">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button key={star} type="button" onClick={() => setRating(star)} className={`transition-all hover:scale-125 ${star <= rating ? "text-amber-500" : "text-zinc-800"}`}>
                      <Star className="w-8 h-8 md:w-12 md:h-12" fill={star <= rating ? "currentColor" : "none"} />
                    </button>
                  ))}
                </div>
                <p className="text-center text-[10px] md:text-xs font-black text-zinc-500 uppercase tracking-widest">{rating === 1 ? "Sangat Buruk" : rating === 2 ? "Buruk" : rating === 3 ? "Biasa Saja" : rating === 4 ? "Puas" : "Sangat Puas"}</p>
              </div>
              ...
              <button
                ...
                className="w-full py-3.5 md:py-5 bg-[#2563EB] hover:bg-[#1D4ED8] text-[#FFFFFF] font-black rounded-2xl md:rounded-3xl transition-all uppercase tracking-[0.1em] md:tracking-[0.15em] flex items-center justify-center gap-2.5 disabled:opacity-50 disabled:cursor-not-allowed text-xs md:text-sm"
              >
                {isSubmitting ? (
            ...
            <form onSubmit={handleSubmitComplain} className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-500">
              <div className="p-6 bg-red-500/5 border border-red-500/20 rounded-3xl flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-zinc-400 leading-relaxed">Pastikan Anda memiliki bukti foto/video unboxing yang jelas. Komplain tanpa bukti yang kuat dapat ditolak oleh sistem moderasi.</p>
                </div>
              </div>

              <div className="space-y-6">
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest flex items-center gap-2">
                  <Camera size={14} className="text-red-500" /> Upload Bukti Foto (Maks 1 MB)
                </label>
                <div className="flex flex-col items-center justify-center">
                  {complaintImagePreview ? (
                    <div className="relative group w-full max-w-md aspect-video rounded-3xl overflow-hidden border-2 border-red-500/50">
                      <img src={complaintImagePreview} className="w-full h-full object-cover" alt="Preview" />
                      <button
                        type="button"
                        onClick={() => {
                    ...
                    <label className="w-full max-w-md aspect-video rounded-3xl border-2 border-dashed border-zinc-800 bg-zinc-950/50 flex flex-col items-center justify-center gap-4 cursor-pointer hover:border-red-500/50 hover:bg-red-500/5 transition-all group">
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-[10px] text-zinc-600 font-medium">JPG, PNG, WEBP, GIF (Maks. 1 MB)</p>
                      </div>
              ...
              <button
                type="submit"
                disabled={isSubmitting}
                className="w-full py-3.5 md:py-5 bg-red-500 hover:bg-red-400 text-white font-black rounded-2xl md:rounded-3xl transition-all uppercase tracking-[0.1em] md:tracking-[0.15em] flex items-center justify-center gap-2.5 disabled:opacity-50 disabled:cursor-not-allowed text-xs md:text-sm"
              >
                {isSubmitting ? (
      ...
      <div className="p-8 bg-zinc-950/30 rounded-[2.5rem] border border-zinc-800 border-dashed text-center">
        <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest italic leading-relaxed">*Harap berhati-hati dalam memberikan penilaian. Review Anda akan tampil di profil toko dan membantu pembeli lain.</p>
      </div>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-6">
          ...
          <div className="relative bg-zinc-900 border border-zinc-800 w-full max-w-md p-8 md:p-10 rounded-[2rem] md:rounded-[3rem] text-center animate-in zoom-in-95 duration-300">
            <div className={`w-16 h-16 md:w-24 md:h-24 mx-auto mb-6 md:mb-8 rounded-2xl md:rounded-[2rem] flex items-center justify-center ${modalConfig.type === "error" ? "bg-red-500/10 text-red-500" : modalConfig.type === "complaint" ? "bg-amber-500/10 text-amber-500" : "bg-blue-500/10 text-blue-500"}`}>
              {modalConfig.type === "error" ? <XCircle className="w-8 h-8 md:w-12 md:h-12" /> : modalConfig.type === "complaint" ? <ShieldAlert className="w-8 h-8 md:w-12 md:h-12" /> : <CheckCircle2 className="w-8 h-8 md:w-12 md:h-12" />}
            </div>
            <h2 className="text-xl md:text-2xl font-black text-white uppercase tracking-tight mb-3 md:mb-4">{modalConfig.title}</h2>
            <p className="text-zinc-400 text-xs md:text-sm leading-relaxed mb-6 md:mb-8 font-medium">{modalConfig.message}</p>
            <button
              onClick={() => {
              ...
              className={`w-full py-4 md:py-5 rounded-xl md:rounded-2xl font-black uppercase tracking-widest transition-all text-xs md:text-sm ${
                modalConfig.type === "error" ? "bg-zinc-800 text-white hover:bg-zinc-700" : modalConfig.type === "complaint" ? "bg-amber-500 text-zinc-950 hover:bg-amber-400" : "bg-blue-500 text-zinc-950 hover:bg-blue-400"
              }`}
            >
