...
export default function PesananPage() {
  ...
  const filteredOrders = (() => {
    ...
    return activeOrders.filter((order) => {
      const matchesTab = activeTab === "semua" || (activeTab === "proses_transaksi" && !["cancelled"].includes(order.status)) || (activeTab === "processing" && ["processing", "waiting_shipment", "payment_verified"].includes(order.status)) || order.status === activeTab;

      const productName = order.product?.name || "";
      const orderId = order.order_id || "";
      const matchesSearch = productName.toLowerCase().includes(searchQuery.toLowerCase()) || orderId.toLowerCase().includes(searchQuery.toLowerCase());
                            ...
                            {[
                              ...
                              return (
                                <div key={s.id} className="flex-1 flex flex-col gap-1.5">
                                  ...
                                  <span className={`text-[7px] font-black uppercase tracking-tighter text-center ${isDone ? "text-emerald-500" : "text-zinc-600"} ${isCurrent ? "animate-pulse" : ""}`}>{s.label}</span>
                                </div>
                              );
                            })}
                          </div>
          ...
          <div className="relative w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-[2.5rem] p-8 animate-in zoom-in-95 duration-300 overflow-hidden">
            ...
            <div className="text-center space-y-4">
              ...
              <div className="py-6 flex justify-center gap-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button key={star} onClick={() => setRatingModal((prev) => ({ ...prev, rating: star }))} className={`transition-all transform hover:scale-125 ${star <= ratingModal.rating ? "text-amber-400" : "text-zinc-800"}`}>
