...
function AuctionCard({ listing, currentUserId }) {
  ...
  useEffect(() => {
    const updateTimer = () => {
      ...
      const start = listing.start_date
        ? new Date(listing.start_date).getTime()
        : 0;
    ...
    switch (userStatus) {
      ...
      case "outbid":
        return {
          ...
          description:
            "Penawaran Anda telah dilompati! Pasang bid baru untuk memimpin.",
        };
      case "won":
        return {
          label: "Menang",
          badge: "bg-emerald-950/80 text-emerald-400 border-emerald-500/30",
          description:
            "Selamat! Anda memenangkan lelang. Selesaikan pembayaran di tab Pesanan Aktif.",
        };
      case "lost":
        return {
          label: "Kalah",
  ...
  return (
    <div className="bg-zinc-900/20 border border-zinc-800 hover:border-zinc-700 rounded-3xl overflow-hidden transition-all group p-4 md:p-6 flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-6 animate-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="w-24 h-24 md:w-28 md:h-28 rounded-2xl overflow-hidden bg-zinc-800 shrink-0 border border-zinc-700 relative mx-auto md:mx-0">
        ...
        />
        <div
          className={`absolute top-2 right-2 px-2.5 py-0.5 rounded-full border text-[8px] font-black uppercase tracking-wider ${statusStyle.badge} backdrop-blur-md`}
        >
          {statusStyle.label}
        </div>
      </div>
      ...
      <div className="flex-1 min-w-0 text-center md:text-left">
        ...
        <h3 className="text-base md:text-xl font-black text-white mb-2 line-clamp-2">
          {listing.name}
        </h3>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4 bg-zinc-950/20 p-3 rounded-2xl border border-zinc-800/50">
          ...
          <div>
            ...
            <p className="text-sm md:text-base font-black text-emerald-500">
              {formatPrice(listing.user_max_bid)}
            </p>
          </div>
      ...
      <div className="flex flex-row md:flex-col justify-center items-stretch gap-3 w-full md:w-auto shrink-0 self-stretch md:self-start">
        <Link
          href={`/toko/detail-lelang/${listing.id}`}
          className="flex-1 md:flex-none px-6 py-3.5 bg-zinc-900 border border-zinc-800 hover:border-zinc-700 text-zinc-300 hover:text-white rounded-2xl transition-all text-xs font-black flex items-center justify-center gap-2 active:scale-95"
        >
...
export default function LelangPage() {
  ...
  const filteredListings = listings.filter((item) => {
    ...
    if (activeTab === "berjalan") {
      matchesTab =
        item.user_status === "highest" || item.user_status === "outbid";
  ...
  const getTabCount = (tabId) => {
    ...
    if (tabId === "menang") {
      return activeListings.filter(
        (item) => item.user_status === "won" || item.user_status === "highest",
      ).length;
    }
    if (tabId === "kalah") {
      return activeListings.filter(
        (item) => item.user_status === "lost" || item.user_status === "outbid",
      ).length;
    }
    return 0;
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-zinc-400 font-medium">
            Pantau semua lelang yang sedang Anda ikuti dan tawaran Anda
          </p>
        </div>
        <div>
          <Link
          ...
          >
            ...
            <ArrowRight
              size={14}
              className="group-hover:translate-x-1 transition-transform text-emerald-500"
            />
          </Link>
        </div>
      </div>
        ...
        <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-2">
          {[
            ...
            >
              <span>{tab.label}</span>
              <span
                className={`flex items-center justify-center w-5 h-5 rounded-full text-[9px] font-black ${activeTab === tab.id ? "bg-zinc-950/20 text-zinc-950" : "bg-zinc-800 text-zinc-400"}`}
              >
                {getTabCount(tab.id)}
              </span>
            </button>
      ...
      <div className="space-y-4">
        {isLoading ? (
          [1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-44 bg-zinc-900/50 border border-zinc-800 rounded-3xl animate-pulse"
            />
          ))
          ...
          <div className="py-20 flex flex-col items-center text-center space-y-6 bg-zinc-900/20 border border-zinc-800 rounded-[3rem] border-dashed">
            ...
            <div className="space-y-2">
              ...
              <p className="text-zinc-500 max-w-xs mx-auto text-sm">
                {searchQuery
                  ? "Lelang yang Anda cari tidak ditemukan."
                  : "Anda belum memiliki riwayat keikutsertaan lelang pada kategori ini."}
              </p>
            </div>
          </div>
        )}
