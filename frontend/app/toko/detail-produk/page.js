    ...
    return (
        <div className="min-h-screen bg-zinc-50 pt-20 lg:pt-28 pb-6 flex flex-col">
            <div className="max-w-[1440px] mx-auto px-4 w-full flex-1 flex flex-col">
                ...
                <div className="bg-white border border-zinc-200 rounded-[2rem] lg:rounded-[2.5rem] overflow-hidden shadow-xl flex flex-col lg:flex-row lg:flex-1 lg:min-h-0 mb-4">
                        ...
                        {selectedProduct.images?.length > 1 && (
                            <div className="absolute inset-x-4 top-1/2 -translate-y-1/2 flex justify-between pointer-events-none">
                                <button
                                    ...
                                    className="w-10 h-10 rounded-xl bg-white/90 backdrop-blur-md shadow-lg border border-zinc-200/50 flex items-center justify-center text-zinc-800 hover:bg-emerald-500 hover:text-white transition-all pointer-events-auto active:scale-90"
                                >
                                    <ChevronLeft size={20} />
                                </button>
                                <button
                            ...
                            <div className="absolute bottom-6 inset-x-0 flex justify-center gap-2 px-4 overflow-x-auto no-scrollbar">
                                {selectedProduct.images.map((img, idx) => (
                                    <button
                                        ...
                                        className={`w-14 h-14 rounded-xl border-2 overflow-hidden transition-all flex-shrink-0 bg-white shadow-md ${activeImageIndex === idx ? 'border-emerald-500 scale-105' : 'border-white opacity-60 hover:opacity-100'}`}
                                    >
                        ...
                        <div className="space-y-6 lg:space-y-8">
                            <div>
                                <div className="flex items-center gap-2 mb-3">
                                    <span className="bg-emerald-100 text-emerald-700 text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md border border-emerald-200">
                                    ...
                                    <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md border ${selectedProduct.type === 'sell' ? 'bg-blue-100 text-blue-700 border-blue-200' : 'bg-amber-100 text-amber-700 border-amber-200'}`}>
                                        {selectedProduct.type === 'sell' ? 'Penjualan Langsung' : 'Lelang'}
                                    </span>
                                    <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md border ${selectedProduct.type === 'sell' ? 'bg-blue-100 text-blue-700 border-blue-200' : 'bg-amber-100 text-amber-700 border-amber-200'}`}>
                            ...
                            <div className="bg-zinc-50 rounded-[1.5rem] lg:rounded-[2rem] p-5 lg:p-8 border border-zinc-100 space-y-6">
                                <div className="flex flex-wrap items-center justify-between gap-8">
                                    <div>
                                        {/* <p className="text-[9px] font-black text-zinc-500 uppercase tracking-widest mb-1">
                                            {selectedProduct.type === 'sell' ? 'Harga Jual' : 'Current Bid'}
                                        </p> */}
                                        <p className="text-3xl lg:text-3xl font-black text-emerald-600">
                                            {formatPrice(selectedProduct.type === 'sell' ? selectedProduct.price : (selectedProduct.current_bid || selectedProduct.start_bid))}
                                        </p>
                                    </div>
                                        ...
                                        <div className="flex flex-col sm:flex-row items-start gap-4 w-full sm:w-auto">
                                            <div className="flex flex-col gap-1 w-full sm:w-auto">
                                                <div className="flex items-center bg-white border border-zinc-200 rounded-lg overflow-hidden h-12 w-full">
                                                    <button
                                                        ...
                                                        className="px-4 hover:bg-zinc-50 text-zinc-400 font-bold transition-colors"
                                                    >-</button>
                                                    <div className="flex-1 sm:w-20 text-center text-base font-black border-x border-zinc-200 flex items-center justify-center h-full text-zinc-800">
                                                        {buyQuantity}
                                                    </div>
                                                    <button
                                                        onClick={() => setBuyQuantity(prev => Math.min(selectedProduct.stock, prev + 1))}
                                                        className="px-4 hover:bg-zinc-50 text-zinc-400 font-bold transition-colors"
                                                    >+</button>
                                                </div>
                                            ...
                                            <div className="flex items-center gap-2 h-12 w-full">
                                                <button
                                                    ...
                                                    className="flex-1 bg-white hover:bg-zinc-50 border-2 border-emerald-500 text-emerald-600 font-black px-4 rounded-xl text-xs transition-all active:scale-95 disabled:opacity-50 h-full flex items-center justify-center"
                                                >
                                                ...
                                                <button
                                                    ...
                                                    disabled={isBuying}
                                                    className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white font-black px-6 rounded-xl text-xs transition-all active:scale-95 disabled:opacity-50 shadow-lg shadow-emerald-500/20 h-full flex items-center justify-center"
                                                >
                                ...
                                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                                    <div className="flex items-center gap-4 w-full sm:w-auto">
                                        <div className="w-11 h-11 rounded-xl bg-zinc-100 overflow-hidden border border-zinc-200 shadow-sm flex-shrink-0">
                                            {selectedProduct.shop?.logo_url ? (
                                                <img src={selectedProduct.shop.logo_url} className="w-full h-full object-cover" alt={selectedProduct.shop.name} />
                                            ) : (
                                    ...
                                    <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto border-t sm:border-t-0 pt-4 sm:pt-0 border-zinc-100">
                                        {selectedProduct.shop?.whatsapp && (
                                            <a
                                                ...
                                                className="flex items-center justify-center gap-2 text-[11px] font-black text-white bg-[#25D366] hover:bg-[#128C7E] transition-all py-2.5 px-4 rounded-xl shadow-md shadow-green-500/10"
                                            >
                                        ...
                                        <button
                                            onClick={handleShare}
                                            className="flex items-center justify-center gap-2 text-[11px] font-black text-zinc-600 hover:text-emerald-600 transition-colors py-2.5 px-4 rounded-xl bg-white border border-zinc-200 hover:border-zinc-300"
                                        >
                                ...
                                {(selectedProduct.is_free_shipping || selectedProduct.is_free_packing) && (
                                    <div className="flex flex-wrap gap-3 pt-1">
                                        {selectedProduct.is_free_shipping && (
                                            <div className="flex-1 min-w-[130px] p-4 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-2xl flex items-center gap-3 shadow-sm">
                                        ...
                                        {selectedProduct.is_free_packing && (
                                            <div className="flex-1 min-w-[130px] p-4 bg-blue-50 text-blue-700 border border-blue-100 rounded-2xl flex items-center gap-3 shadow-sm">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4">
                                <div className="bg-zinc-50 border border-zinc-100 rounded-2xl p-5 shadow-sm hover:border-emerald-500/30 transition-all group/spec">
                                ...
                                <div className="bg-zinc-50 border border-zinc-100 rounded-2xl p-5 shadow-sm hover:border-emerald-500/30 transition-all group/spec">
                                    <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest leading-none mb-3">ID Produk</p>
                                    <p className="text-[12px] font-black text-zinc-900 leading-snug">{selectedProduct.product_id || '-'}</p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="text-sm text-zinc-600 leading-relaxed description-content bg-zinc-50 p-6 rounded-2xl border border-zinc-100" dangerouslySetInnerHTML={{ __html: selectedProduct.description || 'Tidak ada deskripsi.' }}></div>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="text-sm text-zinc-600 leading-relaxed description-content bg-zinc-50 p-6 rounded-2xl border border-zinc-100" dangerouslySetInnerHTML={{ __html: selectedProduct.shipping_description || 'Tidak ada informasi pengiriman.' }}></div>
                            </div>
