...
function DetailContent() {
  ...
  const fetchProductDetail = async (id) => {
    try {
      ...
      if (response.ok && result.data) {
        ...
        const currentBid = Number(result.data.current_bid) || Number(result.data.start_bid) || 0;
  ...
  const fetchBids = async () => {
    if (!productId) return [];
    try {
      const response = await fetch(`${getApiUrl()}/listings/${productId}/bids`);
      const result = await response.json();
      if (response.ok && result.data) {
        setBids(result.data);
        return result.data;
  ...
  useEffect(() => {
    ...
    newSocket.on("receive_bid", (newBid) => {
      setBids((prev) => {
        ...
        return updated.sort((a, b) => Number(b.bid_amount) - Number(a.bid_amount));
      });
      ...
      setSelectedProduct((prev) => {
        ...
        return {
          ...prev,
          current_bid: Math.max(Number(prev.current_bid || 0), Number(newBid.bid_amount)),
        };
      });
    });
    ...
    newSocket.on("auction_ended", async (data) => {
      if (data && String(data.listing_id) === String(productId)) {
        ...
        if (activeUser) {
          ...
          // If current user is the seller
            ...
            setActionModal({
              ...
              message: data.winner_id ? "Lelang Anda telah berakhir dan pemenang telah ditentukan. Menunggu pembeli melengkapi alamat pengiriman." : "Lelang Anda telah berakhir tanpa adanya penawaran.",
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const start = selectedProduct.start_date ? new Date(selectedProduct.start_date).getTime() : 0;
      const end = selectedProduct.end_date ? new Date(selectedProduct.end_date).getTime() : 0;
  ...
  const minNextBid = selectedProduct ? (bids.length > 0 ? Number(bids[0].bid_amount) : Number(selectedProduct.start_bid) || 0) + (Number(selectedProduct.multiple) || 0) : 0;
  ...
  const handleBidInputChange = (val) => {
    ...
    const formatted = new Intl.NumberFormat("id-ID").format(parseInt(cleanVal, 10));
  ...
  const handleSubmitBid = async (e) => {
    ...
    if (!currentUser) {
      setActionModal({
        ...
        type: "danger",
        title: "Perlu Login",
        message: "Silakan login terlebih dahulu untuk memasang penawaran lelang.",
      ...
      if (response.ok) {
        ...
        if (isIncomplete) {
          setActionModal({
            ...
            message: "Anda wajib melengkapi data profil terlebih dahulu (Nama, No. WhatsApp, Alamat, Kota, Provinsi) di halaman pengaturan sebelum dapat memasang penawaran lelang.",
    ...
    if (selectedProduct.user_id === currentUser.id) {
      setActionModal({
        ...
        type: "danger",
        title: "Peringatan",
        message: "Anda tidak dapat memasang penawaran pada produk lelang Anda sendiri.",
        onConfirm: null,
      });
      return;
    }
    ...
    const baseBid = bids.length > 0 ? Number(bids[0].bid_amount) : Number(selectedProduct.start_bid);
    ...
    try {
      ...
      if (response.ok) {
        ...
        setActionModal({
          ...
          message: result.message || "Terjadi kesalahan saat memasang penawaran Anda.",
          onConfirm: null,
        });
      }
    } catch (err) {
      ...
      if (response.ok) {
        ...
        if (isIncomplete) {
          setActionModal({
            ...
            type: "danger",
            title: "Profil Belum Lengkap",
            message: "Anda wajib melengkapi data profil terlebih dahulu (Nama, No. WhatsApp, Alamat, Kota, Provinsi) di halaman pengaturan sebelum dapat melakukan pembelian BIN lelang.",
            confirmText: "Lengkapi Profil",
            cancelText: "Batal",
            onConfirm: () => {
              window.location.href = "/user/pengaturan";
  ...
  const executeBINPurchase = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setAuctionStatus("ended");
        setTimeLeft(null);
        setSelectedProduct((prev) => (prev ? { ...prev, status: "ended", end_date: new Date() } : null));

        setActionModal({
          isOpen: true,
          type: "success",
          title: "Pembelian BIN Berhasil!",
          message: "Pesanan BIN lelang Anda berhasil dibuat. Silakan lengkapi data pengiriman untuk memproses transaksi.",
          onConfirm: () => {
        ...
        setActionModal({
          ...
          message: data.message || "Terjadi kesalahan saat memproses pembelian BIN Anda.",
          onConfirm: null,
        });
      }
    } catch (err) {
    ...
    try {
      ...
      if (response.ok && result.data) {
        setActionModal({
          ...
          message: "Kemenangan lelang Anda berhasil diproses. Silakan lengkapi data pengiriman untuk memproses transaksi.",
          onConfirm: () => {
        ...
        setActionModal({
          ...
          message: result.message || "Terjadi kesalahan saat memproses transaksi lelang Anda.",
          onConfirm: null,
        });
      }
    } catch (err) {
  ...
  const handleShare = async () => {
    ...
    if (success) {
      setActionModal({
        ...
        message: "Tautan lelang telah disalin ke clipboard Anda. Silakan bagikan ke teman Anda!",
        onConfirm: null,
      });
    } else {
      setActionModal({
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-50">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="font-bold text-zinc-500 animate-pulse uppercase tracking-widest text-xs">Memuat Detail Lelang...</p>
        </div>
      </div>
    );
  }
  ...
  const baseBid = selectedProduct ? (bids.length > 0 ? Number(bids[0].bid_amount) : Number(selectedProduct.start_bid)) : 0;
  ...
  return (
    <div className="min-h-screen bg-zinc-50 pt-20 lg:pt-28 pb-12 flex flex-col">
      <div className="max-w-[1440px] mx-auto px-4 w-full flex-1 flex flex-col">
        {/* Back Link */}
        <Link href="/lelang" className="inline-flex items-center gap-2 text-zinc-500 hover:text-amber-600 font-bold mb-4 transition-colors group text-sm">
        ...
        <div className="bg-white lg:border lg:border-zinc-200 rounded-none lg:rounded-[2.5rem] overflow-hidden flex flex-col lg:flex-row lg:flex-1 lg:min-h-0 mb-4">
          ...
          <div className="lg:w-[45%] bg-white relative group border-b lg:border-b-0 lg:border-r border-zinc-200 flex flex-col justify-center">
            {parsedImages.length > 0 && parsedImages[activeImageIndex] ? (
              <img src={parsedImages[activeImageIndex]} className="w-full max-h-[320px] sm:max-h-[500px] lg:max-h-none aspect-square lg:aspect-auto object-contain p-4 sm:p-8 lg:p-20 transition-transform duration-700 group-hover:scale-105 cursor-zoom-in" alt={selectedProduct.name} onClick={() => setIsImageZoomed(true)} />
            ) : (
            ...
            {parsedImages.length > 1 && (
              <div className="relative lg:absolute lg:bottom-6 bottom-4 inset-x-0 flex justify-center gap-2 px-4 overflow-x-auto no-scrollbar py-2">
                {parsedImages.map((img, idx) => (
                  <button key={idx} onClick={() => setActiveImageIndex(idx)} className={`w-12 h-12 sm:w-14 sm:h-14 rounded-xl border-2 overflow-hidden transition-all flex-shrink-0 bg-white ${activeImageIndex === idx ? "border-amber-500 scale-105" : "border-zinc-200 opacity-60 hover:opacity-100"}`}>
                ...
                <div className="flex items-center gap-4 w-full sm:w-auto">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-zinc-500 flex items-center gap-1 font-bold truncate">
                      <MapPin size={12} className="text-zinc-400 flex-shrink-0" /> <span className="truncate">{selectedProduct.shop?.city || "Lokasi tidak diset"}</span>
                    </p>
                  </div>
                </div>
              ...
              <div className="bg-transparent lg:bg-white rounded-none lg:rounded-[2rem] p-0 sm:p-0 lg:p-8 border-0 lg:border lg:border-zinc-200 space-y-6 relative overflow-hidden">
                ...
                <div className="flex flex-col items-center justify-center text-center py-4 bg-amber-50/50 border border-amber-200/60 rounded-2xl">
                  {auctionStatus === "scheduled" && (
                    <>
                      <p className="text-[10px] font-black text-amber-700 uppercase tracking-widest mb-2.5 flex items-center gap-1.5">
                        <Calendar size={12} className="animate-bounce" /> Lelang Dimulai Dalam
                      </p>
                      {timeLeft && (
                  ...
                  {auctionStatus === "active" && (
                    <>
                      <p className="text-[10px] font-black text-amber-700 uppercase tracking-widest mb-2.5 flex items-center gap-1.5">
                        <Clock size={12} className="animate-spin duration-3000" /> Sisa Waktu Lelang
                      </p>
                      {timeLeft ? (
                        <div className="flex items-center justify-center gap-1.5 sm:gap-2.5">
                          {timeLeft.d !== "0" && (
                        ...
                        <span className="text-sm font-black text-zinc-500">MEMPROSES...</span>
                      )}
                    </>
                  )}
                ...
                <div className="grid grid-cols-3 gap-2 sm:gap-4 pt-4 border-t border-zinc-150">
                  <div className="text-center p-2 border-r border-zinc-100 flex flex-col justify-center">
                    <p className="text-[8px] sm:text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-1 leading-none">Tawaran Awal (OB)</p>
                    <p className="text-[10px] xs:text-xs sm:text-base md:text-lg font-black text-zinc-800 tracking-tighter xs:tracking-tight">{formatPrice(selectedProduct.start_bid)}</p>
                  </div>
                  <div className="text-center p-2 border-r border-zinc-100 flex flex-col justify-center">
                    <p className="text-[8px] sm:text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-1 leading-none">Kelipatan</p>
                    <p className="text-[10px] xs:text-xs sm:text-base md:text-lg font-black text-amber-600 tracking-tighter xs:tracking-tight">+{formatPrice(selectedProduct.multiple)}</p>
                  </div>
                  <div className="text-center p-2 flex flex-col justify-center">
                    <p className="text-[8px] sm:text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-1 leading-none">Beli Sekarang (BIN)</p>
                    {selectedProduct.bin_price ? (
                      <p className="text-[10px] xs:text-xs sm:text-base md:text-lg font-black text-emerald-600 tracking-tighter xs:tracking-tight">{formatPrice(selectedProduct.bin_price)}</p>
                    ) : (
                      <p className="text-[8px] xs:text-[10px] sm:text-xs font-black text-zinc-400 uppercase tracking-widest italic leading-none pt-0.5">Tidak Ada</p>
                    )}
                  </div>
                </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {/* {bids.length > 0 && (
                        <span className="text-[10px] font-black text-[#2b4c7e] bg-blue-50 px-2 py-0.5 rounded-md border border-blue-100 uppercase tracking-widest animate-pulse flex items-center gap-1">
                          <span className="w-1.5 h-1.5 bg-[#2b4c7e] rounded-full animate-ping"></span> Langsung
                        </span>
                      )} */}
                    </div>
                            ...
                            <div key={bid.id} className={`flex items-center justify-between p-3.5 transition-colors ${isHighest ? "bg-[#2b4c7e]/5 hover:bg-[#2b4c7e]/10" : "hover:bg-zinc-50/50"}`}>
                              <div className="flex items-center gap-3 min-w-0">
                                <div className={`w-8 h-8 rounded-xl overflow-hidden flex items-center justify-center text-xs font-black shrink-0 border ${isHighest ? "bg-[#2b4c7e] text-white border-transparent" : "bg-white text-zinc-600 border-zinc-200"}`}>
                                  ...
                                  <span className={bid.bidder?.avatar_url ? "hidden" : ""}>{bidderName.charAt(0).toUpperCase()}</span>
                                </div>
                                <div className="min-w-0">
                                  <p className="text-sm font-black text-[#2b4c7e] truncate leading-tight">{bidderName}</p>
                                  <p className="text-[10px] font-bold text-zinc-400 mt-0.5">{formatBidDate(bid.created_at)}</p>
                                </div>
                              </div>
                    ...
                    <div className="bg-zinc-50 border border-zinc-200 p-5 rounded-3xl space-y-4">
                      ...
                      <div className="flex items-center justify-between border-b border-zinc-200 pb-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <h4 className="text-xs font-black text-zinc-900 uppercase tracking-wider">Pasang Penawaran Anda</h4>
                      ...
                      <div className="text-[10px] font-bold px-1">
                        {validation.isValid === null && (
                          <span className="text-zinc-500">
                            Minimal bid berikutnya: <span className="text-zinc-900 font-black">{formatPrice(minBid)}</span> (Kelipatan +{formatPrice(multiple)})
                          </span>
                        )}
                        {validation.isValid === false && <span className="text-rose-600">⚠️ {validation.message}</span>}
                        {validation.isValid === true && <span className="text-emerald-600">✓ {validation.message}</span>}
                      </div>
                    </div>
                  )}
                        ...
                        {(() => {
                          ...
                          if (hasOrder) {
                            ...
                            return (
                              <div className="pt-3 border-t border-zinc-200/50 space-y-3">
                                <p className="text-xs text-zinc-600 font-bold leading-relaxed">
                                  Pemenang Lelang: <span className="text-[#2b4c7e] font-black uppercase">{winnerName}</span> dengan harga <span className="text-emerald-600 font-black text-sm">{formatPrice(winnerPrice)}</span>
                                </p>
                                ...
                                {/* Visitor Info */}
                                {(!currentUser || (String(winnerUserId) !== String(currentUser.id) && String(selectedProduct.user_id) !== String(currentUser.id))) && <p className="text-[10px] text-zinc-400 font-medium italic mb-0">Transaksi sedang diproses oleh pemenang lelang dan penjual.</p>}
                              </div>
                            );
                          } else if (hasBids) {
                            ...
                            return (
                              <div className="pt-3 border-t border-zinc-200/50 space-y-3">
                                <p className="text-xs text-zinc-600 font-bold leading-relaxed">
                                  Pemenang Lelang: <span className="text-[#2b4c7e] font-black uppercase">{winnerName}</span> dengan tawaran <span className="text-emerald-600 font-black text-sm">{formatPrice(winnerPrice)}</span>
                                </p>
                            ...
                            return (
                              <div className="pt-2 border-t border-zinc-200/50">
                                <p className="text-xs text-zinc-400 font-bold mb-0">Lelang berakhir tanpa ada penawaran.</p>
                              </div>
                            );
                          }
                        })()}
                    ...
                    {selectedProduct.is_free_shipping && (
                      <div className="flex-1 min-w-[130px] p-3.5 sm:p-4 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-2xl flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] opacity-70 font-bold leading-none">Ditanggung Penjual</p>
                        </div>
                      </div>
                    )}
