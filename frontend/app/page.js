...
export default function Home() {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      try {
        ...
        if (response.ok) {
          const activeOnes = (result.data || []).filter(
      ...
      await Promise.all(
        productIds.map(async (pid) => {
          try {
            ...
            console.error(
              "Error fetching product chat count from",
              getApiUrl(),
              e,
            );
          }
    ...
    const fetchAdvertisements = async () => {
      try {
        const response = await fetch(
          `${getApiUrl()}/advertisements`,
        );
        const result = await response.json();
        if (response.ok) {
          const berandaAds = (result.data || []).filter(
            (ad) =>
              ad.placement?.toLowerCase() === "beranda" &&
        ...
        console.error(
          "Failed to fetch advertisements from",
          getApiUrl(),
          err,
        );
      }
  ...
  useEffect(() => {
        ...
        setListings((prev) => {
          if (prev.some((item) => String(item.id) === String(newProduct.id))) {
            return prev.map((item) =>
              String(item.id) === String(newProduct.id) ? newProduct : item,
            );
          }
    ...
    socket.on("listing_status_updated", (updated) => {
      ...
      if (updated.status?.toLowerCase() !== "active") {
        setListings((prev) =>
          prev.filter((item) => String(item.id) !== String(dataId)),
        );
      } else {
        setListings((prev) => {
    ...
    socket.on("advertisement_updated", (updatedAd) => {
      setAdvertisements((prev) => {
        const isTargetPlacement =
          updatedAd.placement?.toLowerCase() === "beranda";
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900 selection:bg-emerald-200 font-sans">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 w-full relative z-10 pt-28 pb-14 sm:pt-40 sm:pb-24 lg:pt-52 lg:pb-36">
          <div className="max-w-2xl space-y-8 animate-fade-in-up">
            ...
            <p className="text-base sm:text-lg md:text-xl text-zinc-300 max-w-xl leading-relaxed font-medium">
              Marketplace terpercaya untuk jual beli satwa, perlengkapan, dan
            ...
            <div className="flex flex-col sm:flex-row gap-4 sm:gap-5 pt-4">
              <Link
                href="/toko"
                className="group bg-emerald-500 hover:bg-emerald-400 text-emerald-950 px-6 sm:px-8 py-4 sm:py-5 rounded-2xl font-black text-base sm:text-lg transition-all hover:scale-105 flex items-center justify-center gap-2"
              >
                Mulai Belanja{" "}
                <ArrowRight
                  size={20}
      ...
      {slidesLength > 0 && (
        <section className="bg-zinc-950 py-12 relative overflow-hidden select-none">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
              ...
              <div className="flex gap-2">
                {Array.from({ length: slidesLength }).map((_, idx) => (
                  <button
                    ...
                    className={`h-2 rounded-full transition-all duration-300 ${activeAdIndex === idx ? "w-6 bg-emerald-500" : "w-2 bg-white/20 hover:bg-white/40"}`}
            ...
            <div
              ...
              className="relative w-full max-w-4xl mx-auto aspect-[16/9] sm:aspect-[21/9] flex items-center justify-center overflow-visible"
            >
              {activeSlides.map((ad, idx) => {
                const isActive = idx === activeAdIndex;
                const isLeft =
                  idx === (activeAdIndex - 1 + slidesLength) % slidesLength;
                const isRight = idx === (activeAdIndex + 1) % slidesLength;

                let positionClasses =
                  "z-0 opacity-0 scale-[0.7] translate-x-[50%] pointer-events-none";
                if (isActive) {
                  positionClasses =
                    "z-30 opacity-100 scale-100 translate-x-0 bg-zinc-950 shadow-2xl group";
                } else if (isLeft) {
                  positionClasses =
                    "z-10 opacity-30 scale-[0.82] -translate-x-[15%] sm:-translate-x-[25%] pointer-events-none bg-zinc-950/80";
                } else if (isRight) {
                  positionClasses =
                    "z-10 opacity-30 scale-[0.82] translate-x-[15%] sm:translate-x-[25%] pointer-events-none bg-zinc-950/80";
                }
                ...
                return (
                  <div
                    key={ad.id}
                    className={`absolute w-[85%] sm:w-full h-full rounded-2xl sm:rounded-[2rem] p-5 sm:p-8 flex flex-col justify-center overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] ${positionClasses} ${ad.link_url ? "cursor-pointer" : ""}`}
                  >
                    ...
                    <div
                      className={`absolute -right-20 -bottom-20 w-80 h-80 bg-gradient-to-tr ${ad.glowColor} rounded-full blur-[80px] opacity-25 z-10`}
                    />

                    {/* Entire Widescreen Clickable Link Layer (Opens in new tab) */}
                    <Link
                      href={ad.link_url || "/toko"}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="absolute inset-0 z-30 cursor-pointer"
                      aria-label="Kunjungi link promosi"
                    />
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="flex flex-col items-center text-center space-y-4 mb-10">
            ...
            <h2 className="text-4xl md:text-6xl font-black text-zinc-900 tracking-tight">
              Etalase <span className="text-emerald-600">Satwa </span>
            </h2>
          </div>
          ...
          {auctionListings.length > 0 && (
            <div className="space-y-8 animate-fade-in-up mb-8">
              <div className="flex items-center justify-between bg-white border border-zinc-100 rounded-2xl px-4 py-3 sm:px-6 sm:py-4 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)]">
                <div className="flex items-center gap-3">
                  <h3 className="text-lg sm:text-2xl font-black text-zinc-900 flex items-center gap-2">
                    Lelang
                  </h3>
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => scroll("__auction__", "left")}
                    className="p-2 border border-zinc-200 rounded-xl hover:bg-zinc-50 transition-all"
                  >
                    <ChevronLeft size={20} />
                  </button>
                  <button
                    onClick={() => scroll("__auction__", "right")}
                    className="p-2 border border-zinc-200 rounded-xl hover:bg-zinc-50 transition-all"
                  >
              ...
              >
                {auctionListings.map((product) => (
                  <div
                    key={product.id}
                    className="w-40 sm:w-52 lg:w-[calc((100%-64px)/5)] shrink-0"
                  >
          ...
          {isLoadingListings ? (
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <div
                  key={i}
                  className="bg-white border border-zinc-100 rounded-3xl min-h-[20rem] animate-pulse"
                />
              ))}
            </div>
          ) : (
            <div className="space-y-6">
              {Object.entries(groupedListings)
                .filter(
                  ([catName]) => activeTab === "Semua" || activeTab === catName,
                )
              ...
              {listings.length === 0 && (
                <div className="py-32 text-center bg-white rounded-[3rem] border-2 border-dashed border-zinc-100">
                  ...
                  <p className="text-zinc-500 max-w-md mx-auto text-lg">
                    Saat ini belum ada produk aktif.
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          ...
          <div className="text-center max-w-3xl mx-auto mb-16 md:mb-20">
            ...
            <p className="text-sm md:text-base text-zinc-500 leading-relaxed font-medium px-4 md:px-0">
              SatwaiD hadir sebagai platform jual beli satwa terpercaya yang
              menghubungkan pembeli dengan seller dan breeder berkualitas di
              seluruh Indonesia melalui sistem transaksi yang aman dan
              transparan.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12 lg:gap-16 text-center">
            ...
            <div className="flex flex-col items-center space-y-3 md:space-y-5 group cursor-pointer">
              <div className="relative w-20 h-20 md:w-28 md:h-28 flex items-center justify-center mb-1">
                ...
                <PawPrint
                  size={40}
                  className="w-8 h-8 md:w-10 md:h-10 text-emerald-700 relative z-10 transition-transform group-hover:scale-110 duration-500"
                  strokeWidth={1.5}
                />
              </div>
              <h3 className="text-lg md:text-xl font-black text-zinc-900 transition-colors group-hover:text-emerald-600 duration-300">
                Seller Terpercaya
              </h3>
              <p className="text-xs md:text-[13px] text-zinc-500 leading-relaxed font-medium px-4">
                Ribuan pilihan satwa dan perlengkapan hobi dari seller
                terpercaya.
              </p>
            </div>
            ...
            <div className="flex flex-col items-center space-y-3 md:space-y-5 group cursor-pointer">
              ...
              <p className="text-xs md:text-[13px] text-zinc-500 leading-relaxed font-medium px-2">
                Seluruh transaksi di SatwaiD menggunakan sistem Rekber untuk
                memberikan pengalaman jual beli yang lebih aman, transparan, dan
                terpercaya.
              </p>
            </div>
            ...
            <div className="flex flex-col items-center space-y-3 md:space-y-5 group cursor-pointer">
              ...
              <p className="text-xs md:text-[13px] text-zinc-500 leading-relaxed font-medium px-4">
                Seluruh aktivitas transaksi tercatat secara jelas dan mudah
                dipantau.
              </p>
            </div>
            ...
            <div className="flex flex-col items-center space-y-3 md:space-y-5 group cursor-pointer">
              ...
              <p className="text-xs md:text-[13px] text-zinc-500 leading-relaxed font-medium px-2">
                SatwaiD berkomitmen menjaga perdagangan satwa yang legal dan
                aman.
              </p>
            </div>
          </div>
