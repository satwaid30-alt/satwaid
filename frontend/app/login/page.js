...
export default function LoginPage() {
    ...
    const handleForgotPassword = async (e) => {
        ...
        try {
            ...
            setForgotResult({ temp_password: data.temp_password, username: data.username });
        } catch (err) {
    ...
    const handleCopy = async () => {
        ...
        const success = await copyToClipboard(forgotResult.temp_password);
        if (success) {
            setCopied(true);
    ...
    const handleUsePassword = () => {
        setFormData({ username: forgotResult.username, password: forgotResult.temp_password });
        closeForgotModal();
    };
    ...
    return (
        <div className="min-h-screen bg-[#050505] flex items-center justify-center p-4 relative overflow-hidden font-sans selection:bg-emerald-500/30">
            ...
            {/* ── Login Container ── */}
            <div className="w-full max-w-[1000px] grid md:grid-cols-2 bg-zinc-900/40 backdrop-blur-2xl border border-white/5 rounded-[2.5rem] overflow-hidden shadow-2xl relative z-10">
                ...
                <div className="hidden md:flex flex-col justify-between p-12 relative overflow-hidden bg-emerald-500/5">
                    ...
                    <div className="relative z-10 space-y-6">
                        ...
                        <p className="text-zinc-400 text-lg leading-relaxed max-w-sm">
                            Tempat terpercaya untuk jual beli satwa, perlengkapan, dan komunitas pecinta fauna.
                        </p>
                    </div>
                ...
                <div className="p-8 md:p-12 flex flex-col justify-center bg-black/20">
                    <div className="mb-10">
                        ...
                        <p className="text-zinc-500">Masukkan akun Anda untuk melanjutkan akses penuh.</p>
                    </div>
            ...
            {showModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="bg-zinc-900 border border-emerald-500/30 rounded-[2rem] p-10 max-w-sm w-full text-center relative z-10 shadow-2xl animate-in zoom-in duration-300">
                        <div className="w-24 h-24 bg-emerald-500/20 rounded-full flex items-center justify-center mx-auto mb-8 relative">
                            ...
                            <div className="w-16 h-16 bg-emerald-500 rounded-full flex items-center justify-center text-zinc-950 shadow-lg shadow-emerald-500/40">
            ...
            {showForgotModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="bg-zinc-900 border border-white/5 rounded-[2.5rem] w-full max-w-md relative z-10 shadow-2xl overflow-hidden animate-in zoom-in duration-300">
                        ...
                        <div className="p-8">
                            {forgotResult ? (
                                <div className="space-y-6">
                                    <div className="p-6 bg-zinc-950 border border-emerald-500/20 rounded-2xl text-center space-y-4">
                                        <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Password Sementara</p>
                                        <div className="flex items-center justify-center gap-4">
                                            <code className="text-3xl font-mono font-black text-emerald-400 tracking-[0.2em]">
                                                {forgotResult.temp_password}
                                            </code>
                                            <button
                                                onClick={handleCopy}
                                                className={`p-3 rounded-xl transition-all ${copied ? "bg-emerald-500 text-zinc-950" : "bg-white/5 text-zinc-400 hover:text-white"}`}
                                            >
                                                {copied ? <Check size={18} /> : <Copy size={18} />}
                                            </button>
                                        </div>
                                        <p className="text-xs text-zinc-500">Gunakan ini untuk login, lalu ganti di profil.</p>
                                    </div>
                                    <button
                                        onClick={handleUsePassword}
                                        className="w-full bg-emerald-500 text-zinc-950 font-black py-4 rounded-2xl hover:bg-emerald-400 transition-all"
                                    >
                                        Login Sekarang
                                    </button>
                                </div>
                                ...
                                <form onSubmit={handleForgotPassword} className="space-y-6">
                                    ...
                                    <div className="flex gap-3">
                                        ...
                                        <button
                                            ...
                                            className="flex-1 bg-amber-500 hover:bg-amber-400 text-zinc-950 font-black py-4 rounded-2xl transition-all shadow-xl shadow-amber-500/20 disabled:opacity-50"
                                        >
