            ...
            <div className="w-full max-w-md relative z-10">
                <div className="bg-zinc-900/50 backdrop-blur-2xl border border-zinc-800 rounded-[2.5rem] p-10 md:p-12 shadow-2xl shadow-emerald-500/5">
                    <div className="text-center mb-10">
                        <div className="w-16 h-16 bg-emerald-500 rounded-2xl flex items-center justify-center text-zinc-950 font-black text-3xl mx-auto mb-6 shadow-lg shadow-emerald-500/20">
                            RH
                        </div>
                        <h1 className="text-3xl font-black text-white mb-2 tracking-tight">Admin Portal</h1>
                        <p className="text-zinc-400 font-medium">Masuk untuk mengelola konten Reptile Haven</p>
                    </div>

                    <form onSubmit={handleLogin} className="space-y-6">
                        {error && (
                            <div className="bg-red-500/10 border border-red-500/20 text-red-400 px-4 py-3 rounded-2xl text-sm font-semibold animate-shake">
                                {error}
                            </div>
                        )}
                        ...
                        <button
                            type="submit"
                            disabled={isLoading}
                            className="w-full bg-emerald-500 hover:bg-emerald-400 disabled:bg-emerald-500/50 disabled:cursor-not-allowed text-zinc-950 font-black py-4 rounded-2xl transition-all shadow-lg shadow-emerald-500/20 active:scale-[0.98] flex items-center justify-center gap-3 text-lg"
                        >
                            {isLoading ? (
                    ...
                    <p className="text-center mt-10 text-zinc-500 text-sm font-medium">
                        Lupa akses? <a href="#" className="text-emerald-500 hover:text-emerald-400 underline decoration-2 underline-offset-4">Hubungi IT Developer</a>
                    </p>
                </div>
                
                <p className="text-center mt-8 text-zinc-600 text-xs font-bold uppercase tracking-[0.2em]">
                    &copy; 2026 Reptile Haven Portal v1.0
                </p>
            </div>
