        ...
        <div className="min-h-screen bg-zinc-950 pb-20 animate-in fade-in duration-700">
            ...
            <div className="sticky top-0 z-[100] bg-zinc-950/80 backdrop-blur-md border-b border-zinc-800 p-6 lg:px-10">
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    <Link
                        href="/admin/toko-user/detail-produk"
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <span className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border shadow-sm ${getStatusStyles(listing.status)}`}>
                            {listing.status}
                        </span>
                    </div>
                </div>
                            ...
                            <div className="flex flex-col md:flex-row gap-8 items-start md:items-center pb-8 border-b border-zinc-800">
                                ...
                                <div className="space-y-3 flex-1 min-w-0">
                                    <div className="flex flex-wrap items-center gap-3">
                                        <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${listing.type === 'sell' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-amber-500/10 text-amber-400 border-amber-500/20'}`}>
                                            {listing.type === 'sell' ? 'Jual Langsung' : 'Lelang'}
                                        </span>
                                        {listing.product_id && (
                                            <span className="px-4 py-1.5 rounded-full text-[10px] font-mono font-black text-zinc-500 uppercase tracking-[0.2em] border border-zinc-800 bg-zinc-950/50">
                                                ID: {listing.product_id}
                                            </span>
                                        )}
                                        <span className="px-4 py-1.5 rounded-full text-[10px] font-black text-zinc-400 uppercase tracking-widest border bg-zinc-500/10 border-zinc-500/20">
                                            Arsip History
                                        </span>
                                    </div>
                                    <div>
                                    ...
                                    <div className="flex items-center gap-2 text-zinc-400 text-sm font-medium">
                                        ...
                                        <span>Toko: <strong className="text-zinc-300">{listing.shop?.name}</strong></span>
                                    </div>
                                </div>
                            </div>
                                ...
                                {listing.images?.length > 1 && (
                                    <>
                                        <button
                                            ...
                                            className="absolute left-6 top-1/2 -translate-y-1/2 p-4 bg-zinc-900/50 backdrop-blur-md text-white rounded-2xl hover:bg-emerald-500 hover:text-zinc-950 transition-all opacity-0 group-hover:opacity-100"
                                        >
                                            <ChevronLeft size={28} />
                                        </button>
                                        <button
                                            onClick={() => setActiveImageIndex((prev) => (prev < listing.images.length - 1 ? prev + 1 : 0))}
                                            className="absolute right-6 top-1/2 -translate-y-1/2 p-4 bg-zinc-900/50 backdrop-blur-md text-white rounded-2xl hover:bg-emerald-500 hover:text-zinc-950 transition-all opacity-0 group-hover:opacity-100"
                                        >
                                ...
                                <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-3 p-3 bg-zinc-950/30 backdrop-blur-md rounded-[2rem] border border-white/5">
                                    {listing.images?.map((img, idx) => (
                                        <button
                                            ...
                                            className={`w-14 h-14 rounded-2xl overflow-hidden border-2 transition-all ${activeImageIndex === idx ? 'border-emerald-500 scale-110 shadow-lg shadow-emerald-500/20' : 'border-transparent opacity-50 hover:opacity-100'}`}
                                        >
                            ...
                            <div className="p-10 lg:p-14 space-y-5">
                                ...
                                <div className="space-y-6">
                                    <div className="flex flex-wrap items-center gap-4">
                                        <span className={`px-5 py-2 rounded-full text-[11px] font-black uppercase tracking-widest border ${listing.type === 'sell' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-amber-500/10 text-amber-400 border-amber-500/20'}`}>
                                            {listing.type === 'sell' ? 'Jual Langsung' : 'Lelang'}
                                        </span>
                                        {listing.product_id && (
                                            <span className="px-5 py-2 rounded-full text-[11px] font-mono font-black text-zinc-500 uppercase tracking-[0.2em] border border-zinc-800 bg-zinc-950/50">
                                                ID: {listing.product_id}
                                            </span>
                                        )}
                                    </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    ...
                                    <div className="p-6 bg-zinc-950/30 rounded-[2rem] border border-zinc-800/50">
                                        ...
                                        <p className="text-sm font-black text-white">
                                            {new Date(listing.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
                                        </p>
                                    </div>
                                    <div className="p-6 bg-zinc-950/30 rounded-[2rem] border border-zinc-800/50">
                                        <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2 flex items-center gap-2">
                                        ...
                                        <p className="text-sm font-black text-white">
                                            {listing.shipping_type || 'Belum diatur'}
                                        </p>
                                    </div>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="bg-zinc-950/50 p-8 lg:p-10 rounded-[2rem] lg:rounded-[3rem] border border-zinc-800/50 shadow-inner overflow-hidden">
                                        <div
                                            className="text-base text-zinc-400 leading-relaxed description-content font-medium break-words whitespace-pre-wrap"
                                            dangerouslySetInnerHTML={{ __html: listing.description || "Tidak ada deskripsi." }}
                                        ></div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="bg-zinc-950/50 p-8 lg:p-10 rounded-[2rem] lg:rounded-[3rem] border border-zinc-800/50 shadow-inner overflow-hidden">
                                        <div
                                            className="text-base text-zinc-400 leading-relaxed description-content font-medium break-words whitespace-pre-wrap"
                                            dangerouslySetInnerHTML={{ __html: listing.shipping_description || "Tidak ada informasi pengiriman." }}
                                        ></div>
                                    </div>
                                </div>
                                ...
                                <div className="pt-10 border-t border-zinc-800 space-y-8">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        {listing.status?.toLowerCase() === 'pending' && (
                                            <>
                                                <button
                                                    ...
                                                    className="flex items-center justify-center gap-3 py-6 bg-emerald-500 text-zinc-950 rounded-[2rem] font-black text-sm hover:bg-emerald-400 transition-all shadow-2xl shadow-emerald-500/20 active:scale-95"
                                                >
                                                    <CheckCircle2 size={20} /> Setujui & Verifikasi Produk
                                                </button>
                                                <button
                                                    onClick={() => handleActionClick('reject')}
                                                    className="flex items-center justify-center gap-3 py-6 bg-zinc-800 text-zinc-400 rounded-[2rem] font-black text-sm hover:bg-red-500 hover:text-white transition-all border border-zinc-700 active:scale-95"
                                                >
                                        ...
                                        {listing.status?.toLowerCase() === 'active' && (
                                            <button
                                                onClick={() => handleActionClick('reject')}
                                                className="md:col-span-2 flex items-center justify-center gap-3 py-6 bg-zinc-950 border border-zinc-800 text-zinc-500 rounded-[2rem] font-black text-sm hover:bg-red-500 hover:text-white hover:border-red-500 transition-all active:scale-95 shadow-xl"
                                            >
                                        ...
                                        {listing.status?.toLowerCase() === 'rejected' && (
                                            <button
                                                onClick={() => handleActionClick('verify')}
                                                className="md:col-span-2 flex items-center justify-center gap-3 py-6 bg-zinc-950 border border-zinc-800 text-zinc-400 rounded-[2rem] font-black text-sm hover:bg-emerald-500 hover:text-zinc-950 hover:border-emerald-500 transition-all active:scale-95 shadow-xl"
                                            >
            ...
            <ActionModal
                ...
                title={
                    confirmModal.type === 'verify' ? 'Konfirmasi Verifikasi' :
                        confirmModal.type === 'reject' ? 'Konfirmasi Penolakan' : 'Hapus Permanen'
                }
                message={
                    confirmModal.type === 'verify' ? 'Apakah Anda yakin produk ini sudah layak tayang di marketplace?' :
                        confirmModal.type === 'reject' ? 'Silakan berikan alasan mengapa produk ini tidak layak tayang:' :
                            'Produk ini akan dihapus selamanya dari sistem. Anda yakin?'
                }
