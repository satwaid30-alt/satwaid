...
import { MessageSquare, Bell, Package, CreditCard, Users, Trash2, CheckCircle2, ShieldCheck, AlertCircle, Store } from "lucide-react";
...
export default function Navbar({ theme = "dark", onNotification }) {
    ...
    useEffect(() => {
        if (user && process.env.NEXT_PUBLIC_API_URL) {
            fetch(`${process.env.NEXT_PUBLIC_API_URL}/shops/user/${user.id}`)
                .then(res => res.json())
                .then(res => {
            ...
            <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
                <Link href="/" className="flex items-center group cursor-pointer">
                    <div className="relative w-40 h-11 sm:w-48 sm:h-13 lg:w-56 lg:h-15 group-hover:scale-105 transition-transform duration-300">
                        <Image
                            src={isLightText ? "/images/Logo-Bg-1-2.png" : "/images/Logo-Bg-2-2.png"}
                            alt="SatwaiD Logo"
                            fill
                ...
                <div className={`hidden md:flex items-center gap-8 font-semibold ${isLightText ? "text-white/80" : "text-zinc-500"}`}>
                    {[
                        // { name: "Spesies", href: "/species" },
                        // { name: "Genetik & Morph", href: "/genetics" },
                        { name: "Komunitas", href: "/komunitas" },
                        { name: "Etalase Satwa", href: "/toko" }
                    ].map((item) => {
                        ...
                        return (
                            <Link
                                key={item.name}
                                href={item.href}
                                className={`transition-all relative py-1 hover:text-emerald-500 ${isActive
                                    ? "text-emerald-500 font-bold"
                                    : isLightText ? "hover:text-white" : "hover:text-zinc-900"}`}
                            >
                                {item.name}
                                {isActive && (
                    ...
                    {user ? (
                        <div className="hidden sm:block relative">
                            <button
                                ...
                                className={`flex items-center gap-2 px-4 py-2 rounded-full font-bold transition-all border shadow-sm ${isLightText
                    ...
                    {user && (
                        <div className="relative">
                            <Link
                                ...
                                className={`p-2 rounded-full transition-all relative flex items-center justify-center border shadow-sm ${notifCount > 0
                ...
                <div className="px-6 py-8 flex flex-col gap-5 font-bold">
                    ...
                    {user ? (
                        <div className="sm:hidden border-t border-zinc-100 pt-6 mt-2 flex flex-col gap-4">
                            ...
                            <Link href="/user/pengaturan" className="text-zinc-600 hover:text-emerald-600 transition-colors" onClick={() => setIsMenuOpen(false)}>Pengaturan Akun</Link>
                            <Link href="/user/pesanan" className="text-zinc-600 hover:text-emerald-600 transition-colors" onClick={() => setIsMenuOpen(false)}>Pesanan saya</Link>
                            <Link href="/user/toko/dashboard" className="text-zinc-600 hover:text-emerald-600 transition-colors" onClick={() => setIsMenuOpen(false)}>Dashboard Seller</Link>
                            <button
                                onClick={handleLogout}
