...
export default function Navbar({ theme = "dark", onNotification }) {
  ...
  const isLightText = theme === "dark" && !scrolled && !isMenuOpen && !isMobileLightBg;
  ...
  const fetchCounts = async () => {
    ...
    try {
      ...
      if (res.ok) setNotifCount((result.data.incoming_orders || 0) + (result.data.my_orders || 0) + (result.data.community || 0) + (result.data.chats || 0) + (result.data.system || 0));
    } catch (e) {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 border-b ${scrolled || isMenuOpen || isMobileLightBg ? "bg-white/90 backdrop-blur-md py-3 border-zinc-200" : "bg-transparent py-5 border-transparent"}`}>
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        <Link href="/" className="flex items-center group cursor-pointer">
          <div className="relative w-40 h-11 sm:w-48 sm:h-13 lg:w-56 lg:h-15 group-hover:scale-105 transition-transform duration-300">
            <Image src={isLightText ? "/images/Logo-Bg-1-2.png" : "/images/Logo-Bg-2-2.png"} alt="SatwaiD Logo" fill className="object-contain object-left" />
          </div>
        </Link>

        {/* Desktop Navigation */}
        <div className={`hidden md:flex items-center gap-8 font-semibold ${isLightText ? "text-white/80" : "text-zinc-500"}`}>
          {[
            ...
            return (
              <Link key={item.name} href={item.href} className={`transition-all relative py-1 hover:text-emerald-500 ${isActive ? "text-emerald-500 font-bold" : isLightText ? "hover:text-white" : "hover:text-zinc-900"}`}>
                {item.name}
                {isActive && <span className="absolute -bottom-1 left-0 right-0 h-0.5 bg-emerald-500 rounded-full animate-in fade-in slide-in-from-left-2 duration-300"></span>}
              </Link>
            );
          })}
        </div>
        ...
        <div className="flex items-center gap-4 relative">
            ...
            <div className="relative">
              <Link
                ...
                className={`p-2 rounded-full transition-all relative flex items-center justify-center border shadow-sm ${
                  notifCount > 0 ? "bg-emerald-500 text-white border-emerald-400 shadow-lg shadow-emerald-500/20" : isLightText ? "bg-white/10 text-white border-white/20 hover:bg-white/20" : "bg-white text-zinc-900 border-zinc-200 hover:border-emerald-500 hover:shadow-lg hover:shadow-emerald-500/10"
                }`}
                title="Notifikasi"
              >
                <Bell size={20} />
                {notifCount > 0 && <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-[10px] font-black rounded-full flex items-center justify-center border-2 border-white animate-bounce">{notifCount}</span>}
              </Link>
                ...
                <div className="fixed sm:absolute right-4 sm:right-0 left-4 sm:left-auto mt-3 w-auto sm:w-96 bg-white border border-zinc-100 rounded-[2rem] sm:rounded-[2.5rem] shadow-[0_30px_100px_rgba(0,0,0,0.15)] overflow-hidden animate-in slide-in-from-top-2 fade-in duration-300 z-[60]">
                  <div className="px-5 sm:px-7 py-4 sm:py-6 border-b border-zinc-50 bg-white flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <h3 className="text-lg font-black text-zinc-900">Notifikasi</h3>
                      {notifications.length > 0 && <span className="bg-emerald-500 text-white text-[10px] font-black px-2 py-0.5 rounded-full">{notifications.length}</span>}
                    </div>
                    <button
                  ...
                  <div className="max-h-[480px] overflow-y-auto custom-scrollbar bg-white">
                    {isLoadingNotifs ? (
                      <div className="py-20 flex flex-col items-center gap-3">
                        ...
                        <p className="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em] animate-pulse">Sinkronisasi...</p>
                      </div>
                      ...
                      <div className="divide-y divide-zinc-50">
                        {notifications.map((notif) => {
                          ...
                          if (notif.type === "chat") {
                            ...
                            Icon = Package;
                            iconColor = notif.title.includes("Setuju") ? "bg-emerald-100 text-emerald-600" : "bg-red-100 text-red-600";
                          } else if (notif.type === "moderation_shop") {
                            Icon = Store;
                            iconColor = notif.title.includes("Verifikasi") ? "bg-emerald-100 text-emerald-600" : "bg-amber-100 text-amber-600";
                          ...
                          return (
                            ...
                            >
                              <div className={`shrink-0 w-12 h-12 rounded-2xl ${iconColor} flex items-center justify-center transition-transform group-hover:scale-110 duration-300 shadow-sm`}>
                                <Icon size={20} />
                              </div>
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between mb-1">
                                  <p className="text-sm font-black text-zinc-900 group-hover:text-emerald-600 transition-colors">{notif.title}</p>
                                  <p className="text-[10px] font-bold text-zinc-400 bg-zinc-50 px-2 py-0.5 rounded-md">{new Date(notif.time).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })}</p>
                                </div>
                                <p className="text-xs font-medium text-zinc-500 leading-relaxed mb-1 line-clamp-2">{notif.message}</p>
                                <p className="text-[9px] font-black text-zinc-300 uppercase tracking-widest">{new Date(notif.time).toLocaleDateString("id-ID", { day: "numeric", month: "short" })}</p>
                              </div>
                      ...
                      <div className="py-24 text-center">
                        ...
                        <p className="text-sm font-black text-zinc-400 uppercase tracking-[0.3em]">Hening Sekali...</p>
                        <p className="text-xs font-bold text-zinc-300 mt-2">Belum ada notifikasi baru untuk Anda</p>
                      </div>
                    )}
                  </div>

                  <div className="p-4 sm:p-6 bg-zinc-50/50 border-t border-zinc-50">
                    <Link href="/user/pesanan" onClick={() => setIsChatOpen(false)} className="flex items-center justify-center w-full py-4 bg-zinc-900 hover:bg-emerald-600 text-white text-[11px] font-black uppercase tracking-[0.2em] rounded-2xl transition-all shadow-lg hover:shadow-emerald-500/25 active:scale-[0.98]">
          ...
          {/* Mobile Menu Button */}
          <button onClick={() => setIsMenuOpen(!isMenuOpen)} className={`md:hidden transition-all duration-300 ${user && !isMenuOpen ? "p-0.5" : "p-2 rounded-lg"} ${isLightText ? "text-white hover:bg-white/10" : "text-zinc-900 hover:bg-zinc-100"}`}>
            {isMenuOpen ? (
      ...
      <div className={`md:hidden transition-all duration-500 ease-in-out bg-white border-t border-zinc-100 overflow-y-auto custom-scrollbar ${isMenuOpen ? "max-h-[85vh] opacity-100" : "max-h-0 opacity-0 overflow-hidden"}`}>
        <div className="px-6 py-8 flex flex-col gap-5 font-bold">
          {[
            ...
            const isActive = pathname.startsWith(item.href);
            return (
              <Link key={item.name} href={item.href} onClick={() => setIsMenuOpen(false)} className={`flex items-center justify-between p-3 rounded-2xl transition-all ${isActive ? "bg-emerald-50 text-emerald-600 px-4" : "text-zinc-600 hover:bg-zinc-50"}`}>
                {item.name}
          ...
          {!isLoaded ? (
            ...
            <Link href="/login" className="sm:hidden text-center bg-emerald-600 text-white px-6 py-3 rounded-full font-bold" onClick={() => setIsMenuOpen(false)}>
              Masuk
            </Link>
          )}
        </div>
