      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-12 md:gap-16 mb-20 md:mb-24">
        <div className="col-span-1 sm:col-span-2 space-y-6">
          <Link href="/" className="inline-block">
            <div className="relative w-56 h-14 md:w-72 md:h-20 transition-transform hover:scale-105">
              <Image src="/images/Logo-Bg-1-2.png" alt="SatwaiD Logo" fill className="object-contain object-left" priority />
            </div>
          </Link>
          <p className="text-zinc-400 text-sm md:text-base max-w-xs md:max-w-sm leading-relaxed text-justify">
            SatwaiD adalah platform marketplace inovatif yang menghubungkan penjual satwa terverifikasi dengan para pecinta hewan di seluruh Indonesia. Kami berkomitmen untuk membangun ekosistem jual-beli yang bertanggung jawab, memastikan setiap satwa yang diperdagangkan bukan merupakan spesies dilindungi, serta memberikan keamanan
            transaksi digital yang terjamin melalui identitas yang jelas.
          </p>
          <div className="flex gap-3 pt-2">
            {[{ name: "FB", link: "https://www.facebook.com/SatwaiD/" }].map((social, i) => (
              <a key={i} href={social.link} className="w-9 h-9 border border-white/5 bg-white/5 rounded-xl flex items-center justify-center text-zinc-400 hover:text-emerald-500 hover:border-emerald-500/50 hover:bg-emerald-500/10 transition-all cursor-pointer text-[10px] font-black tracking-tighter">
                {social.name}
              </a>
            ))}
          </div>
        ...
        <div className="space-y-6">
          ...
          <ul className="space-y-3 md:space-y-4 text-zinc-400 font-medium text-sm md:text-base">
            ...
            <li>
              <Link href="/komunitas" className="hover:text-emerald-500 transition-colors">
                Komunitas
              </Link>
            </li>
            <li>
              <a href="/toko" className="hover:text-emerald-500 transition-colors">
                Etalase Satwa
              </a>
            </li>
          </ul>
        ...
        <div className="space-y-6">
          ...
          <ul className="space-y-3 md:space-y-4 text-zinc-400 font-medium text-sm md:text-base">
            <li>
              <Link href="/pusat-informasi?tab=bantuan" className="hover:text-emerald-500 transition-colors">
                Pusat Bantuan
              </Link>
            </li>
            <li>
              <Link href="/pusat-informasi?tab=privasi" className="hover:text-emerald-500 transition-colors">
                Kebijakan Privasi
              </Link>
            </li>
            <li>
              <Link href="/pusat-informasi?tab=syarat" className="hover:text-emerald-500 transition-colors">
                Syarat Penggunaan
              </Link>
            </li>
            <li className="pt-2">
