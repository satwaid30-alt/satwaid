...
const ActionModal = ({
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
      {/* Backdrop */}
      <div className="absolute inset-0 bg-zinc-950/80 backdrop-blur-md animate-in fade-in duration-500" onClick={() => !isLoading && onClose()}></div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 w-full max-w-md rounded-[2.5rem] relative z-10 overflow-hidden shadow-2xl animate-in zoom-in-95 slide-in-from-bottom-4 duration-500">
        {/* Close Button */}
        {!isLoading && (
          <button onClick={onClose} className="absolute top-6 right-6 text-zinc-500 hover:text-white hover:bg-zinc-800 p-2 rounded-full transition-all">
            <X size={18} />
          </button>
        )}
        ...
        <div className="p-10 text-center space-y-8">
          ...
          <div className="space-y-3">
            ...
            <p className="text-zinc-500 text-sm leading-relaxed font-medium px-4">{message}</p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4">
            {onConfirm ? (
              <>
                <button onClick={onClose} disabled={isLoading} className="flex-1 py-4 bg-zinc-800 hover:bg-zinc-700 text-white font-black rounded-2xl transition-all disabled:opacity-50 active:scale-95">
                  {cancelText}
                </button>
                <button onClick={onConfirm} disabled={isLoading} className={`flex-1 py-4 ${config.button} font-black rounded-2xl transition-all shadow-lg disabled:opacity-50 flex items-center justify-center gap-2 active:scale-95`}>
                  {isLoading ? <div className="w-5 h-5 border-2 border-zinc-950/30 border-t-zinc-950 rounded-full animate-spin"></div> : confirmText}
                </button>
              </>
            ) : (
              <button onClick={onClose} className={`w-full py-4 ${config.button} font-black rounded-2xl transition-all shadow-lg active:scale-[0.98]`}>
                Oke, Mengerti
              </button>
            )}
          </div>
