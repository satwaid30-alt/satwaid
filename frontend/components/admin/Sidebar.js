    ...
    return (
        <aside className={`${isCollapsed ? "w-24" : "w-72"} bg-zinc-900 border-r border-zinc-800 flex flex-col h-screen sticky top-0 transition-all duration-300 relative z-40`}>
            ...
            <div className={`p-8 ${isCollapsed ? "px-4 flex justify-center" : ""}`}>
                <Link href="/" className="flex items-center gap-3 group" title={isCollapsed ? "Reptile Haven" : ""}>
                    <div className="w-10 h-10 bg-emerald-500 rounded-xl flex items-center justify-center text-zinc-950 font-bold text-xl shrink-0 group-hover:scale-110 transition-transform">
                        RH
                    </div>
                    {!isCollapsed && (
            ...
            <nav className={`flex-1 ${isCollapsed ? "px-2" : "px-4"} space-y-2 mt-4 overflow-y-auto overflow-x-hidden custom-scrollbar`}>
                {!isCollapsed ? (
                    <p className="px-4 text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-4 truncate">Main Menu</p>
                ) : (
                        ...
                        return (
                            <div key={item.name} className="space-y-1">
                                <button
                                    ...
                                    className={`w-full flex items-center gap-4 ${isCollapsed ? "px-0 justify-center" : "px-4"} py-3 rounded-xl font-semibold transition-all group ${isSubActive
                                    ...
                                    <div className="ml-4 pl-4 border-l border-zinc-800 space-y-1 animate-in slide-in-from-top-2 duration-200">
                                        {item.subMenu.map((sub) => (
                                            <Link
                                                key={sub.name}
                                                href={sub.href}
                                                className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all ${pathname === sub.href
                ...
                <div className={`flex items-center gap-4 mb-6 ${isCollapsed ? "justify-center" : "px-2"}`}>
                    ...
                    {!isCollapsed && (
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-bold truncate">Admin Reptile</p>
                            <p className="text-xs text-zinc-500 truncate">admin@reptilehaven.id</p>
                        </div>
                    )}
                </div>
                <button
                    ...
                    className={`w-full flex items-center justify-center gap-3 ${isCollapsed ? "px-0" : "px-4"} py-3 rounded-xl font-bold text-zinc-400 hover:bg-red-500/10 hover:text-red-500 transition-all group`}
                >
