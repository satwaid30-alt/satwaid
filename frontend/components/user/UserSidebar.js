...
const isSubmenuActive = (subHref, pathname) => {
  ...
  if (subHref === "/user/toko") {
    const siblingSubmenus = [
      "/user/toko/dashboard",
      "/user/toko/jual-produk",
      "/user/toko/lelang-produk",
      "/user/toko/daftar-produk",
      "/user/toko/pesanan-masuk",
      "/user/toko/pengajuan-keuangan",
    ];
    return (
      pathname.startsWith("/user/toko") &&
      !siblingSubmenus.some((sib) => pathname.startsWith(sib))
    );
  }

  if (subHref === "/user/pesanan") {
    return (
      pathname.startsWith("/user/pesanan") &&
      !pathname.startsWith("/user/pesanan/riwayat-pembelian") &&
      !pathname.startsWith("/user/pesanan/lelang")
    );
  }
...
export default function UserSidebar() {
  ...
  const fetchShopStatus = async (userId) => {
    ...
    try {
      ...
      console.error(
        "Error fetching shop status from",
        getApiUrl(),
        err,
      );
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      MENU_ITEMS.forEach((item) => {
        if (
          item.submenu &&
          item.submenu.some((sub) => isSubmenuActive(sub.href, pathname))
        ) {
  ...
  useEffect(() => {
    if (pathname === "/user/komunitas" && user?.id) {
      const markAsRead = async () => {
        try {
          await fetch(
            `${getApiUrl()}/notifications/${user.id}/read`,
            { method: "PUT" },
          );
  ...
  return (
    <>
      {/* --- DESKTOP SIDEBAR --- */}
      <aside
        className={`${isCollapsed ? "w-24" : "w-72"} bg-zinc-900 border-r border-zinc-800 flex-col h-screen sticky top-0 hidden md:flex z-40 transition-all duration-300 relative`}
      >
        ...
        {/* User Profile Header */}
        <div
          className={`p-8 border-b border-zinc-800 pt-12 ${isCollapsed ? "px-4 flex flex-col items-center" : ""}`}
        >
          <div
            className={`flex items-center gap-4 ${isCollapsed ? "" : "mb-2"}`}
          >
            {!isLoaded ? (
              <>
        ...
        {/* Navigation */}
        <nav
          className={`flex-1 ${isCollapsed ? "px-2" : "px-4"} py-6 space-y-1 overflow-y-auto custom-scrollbar overflow-x-hidden`}
        >
          {!isCollapsed ? (
            <p className="px-4 text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-4 truncate">
              Menu Akun
            </p>
          ) : (
              ...
              <div key={item.name} className="space-y-1">
                {hasSubmenu ? (
                  <button
                    ...
                    className={`w-full flex items-center gap-4 ${isCollapsed ? "px-0 justify-center" : "px-4"} py-3 rounded-xl font-semibold transition-all group ${
                      isActive
                ...
                {hasSubmenu && isSubmenuOpen && !isCollapsed && (
                  <div className="ml-12 space-y-1 animate-in slide-in-from-top-2 duration-200">
                    {item.submenu.map((sub) => {
                      ...
                      const isLocked =
                        ((sub.name === "Jual Langsung" ||
                          sub.name === "Lelang Produk") &&
                          shopStatus !== "active") ||
                        ((sub.name === "Dashboard Utama" ||
                          sub.name === "Daftar Produk" ||
                          sub.name === "Pesanan Masuk" ||
                          sub.name === "Pengajuan Keuangan") &&
                          hasNoShop);

                      if (isLocked) {
                        const lockReason = hasNoShop
                          ? "Buka Toko Dahulu"
        ...
        {/* Logout */}
        <div
          className={`p-6 mt-auto border-t border-zinc-800 ${isCollapsed ? "px-2" : ""}`}
        >
          <button
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              ...
              >
                <Bell
                  size={20}
                  className={notifCount > 0 ? "animate-pulse" : ""}
                />
                {notifCount > 0 && (
            ...
            <div className="overflow-y-auto custom-scrollbar space-y-1.5 flex-1 pr-1">
              {isLoadingNotifs ? (
                <div className="py-12 flex flex-col items-center gap-2">
                  ...
                  <p className="text-[8px] font-black text-zinc-500 uppercase tracking-[0.2em] animate-pulse">
                    Sinkronisasi...
                  </p>
                </div>
              ) : notifList.length > 0 ? (
                <div className="divide-y divide-zinc-800/40">
                  {notifList.map((notif) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`shrink-0 w-8 h-8 rounded-lg ${iconColor} flex items-center justify-center transition-transform`}
                        >
