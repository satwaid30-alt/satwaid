  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-[2.5rem] overflow-hidden shadow-2xl">
      ...
      <div className="flex items-center justify-between px-8 py-5 border-b border-zinc-800 bg-zinc-900/80 backdrop-blur-sm sticky top-0 z-10">
        ...
        {hasMany && (
          <button onClick={scrollToBottom} className="flex items-center gap-1.5 px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border border-zinc-700">
      ...
      {hasMany && !isExpanded && (
        <div className="flex flex-col items-center py-5 border-b border-zinc-800 gap-2 bg-zinc-950/30">
          ...
          <button onClick={() => setIsExpanded(true)} className="px-5 py-2 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 hover:text-white text-xs font-black rounded-xl border border-zinc-700 transition-all uppercase tracking-widest">
              ...
              return (
                <div key={comment.id} className={`flex gap-4 group animate-in fade-in slide-in-from-bottom-2 duration-300 ${isMe ? "flex-row-reverse" : ""}`} style={{ animationDelay: `${i * 30}ms` }}>
                  ...
                  <div className={`flex-1 min-w-0 space-y-1.5 ${isMe ? "text-right" : ""}`}>
                    ...
                    <div className={`p-4 border rounded-2xl text-left transition-colors ${isMe ? "bg-emerald-500/10 border-emerald-500/20 rounded-tr-sm group-hover:border-emerald-500/30" : "bg-zinc-950/60 border-zinc-800 rounded-tl-sm group-hover:border-zinc-700"}`}>
          ...
          <form onSubmit={handleSubmit} className="mt-4 space-y-3">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 relative group">
                ...
                />
                <button type="submit" disabled={!newComment.trim()} className="absolute bottom-3 right-3 w-9 h-9 bg-emerald-500 hover:bg-emerald-400 disabled:bg-zinc-700 disabled:text-zinc-500 text-zinc-950 rounded-xl flex items-center justify-center transition-all">
                  <Send size={16} />
                </button>
              </div>
            </div>
