...
export default function Comments({ topicId }) {
    ...
    const fetchComments = async () => {
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/topics/${topicId}/comments`);
                            ...
                            >
                                <div className="w-10 h-10 rounded-2xl overflow-hidden bg-zinc-800 shrink-0 border border-zinc-700">
                                    {comment.author?.avatar_url ? (
                                        <img
                                            src={`${process.env.NEXT_PUBLIC_API_URL}${comment.author.avatar_url}`}
                                            alt={comment.author.username}
                                            className="w-full h-full object-cover"
                                        />
                                    ) : (
                                        <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-emerald-500 to-emerald-700 text-white font-black text-sm">
                                ...
                                <div className="flex-1 min-w-0 space-y-1.5">
                                    <div className="flex items-center gap-2 flex-wrap">
                                        <span className="font-black text-sm text-white">{comment.author?.name || comment.author?.username}</span>
                                    ...
                                    <div className="p-4 bg-zinc-950/60 border border-zinc-800 rounded-2xl rounded-tl-sm group-hover:border-zinc-700 transition-colors">
                ...
                {user ? (
                    <form onSubmit={handleSubmit} className="mt-4 space-y-3">
                        <div className="flex items-start gap-3">
                            <div className="w-9 h-9 rounded-xl overflow-hidden bg-zinc-800 shrink-0 border border-zinc-700 mt-1">
                                {user.avatar_url ? (
                                    <img src={`${process.env.NEXT_PUBLIC_API_URL}${user.avatar_url}`} className="w-full h-full object-cover" />
                                ) : (
                    ...
                    <div className="mt-4 p-5 bg-zinc-950/50 border border-zinc-800 rounded-2xl text-center">
                        <p className="text-zinc-500 text-sm font-medium">
                            Silakan <Link href="/login" className="text-emerald-500 font-black hover:underline">Login</Link> untuk ikut berdiskusi.
                        </p>
                    </div>
                )}
            </div>
