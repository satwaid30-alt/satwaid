        ...
        {allSteps.map((step, i) => {
          ...
          return (
            <div key={i} className={`flex gap-4 items-start relative transition-all duration-500 ${isUpcoming ? "opacity-40 grayscale" : "opacity-100"}`}>
              <div
                className={`w-[24px] h-[24px] rounded-[10px] border-2 flex items-center justify-center z-10 shrink-0 transition-all duration-700 ${
                  step.isCompleted
                    ? isComplainedStep
              ...
              <div className="space-y-1 pt-0.5 flex-1">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-1">
                  <div className="flex items-center gap-2">
                    ...
                    {isActive && <span className="px-1.5 py-0.5 bg-emerald-500/10 border border-emerald-500/20 rounded-[10px] text-[7px] font-black text-emerald-500 uppercase">Aktif</span>}
                    {isCancelledStep && <span className="px-1.5 py-0.5 bg-red-500/10 border border-red-500/20 rounded-[10px] text-[7px] font-black text-red-500 uppercase">Batal</span>}
                  </div>
