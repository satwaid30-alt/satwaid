...
export default function OrderStepper({ order, className = "" }) {
  ...
  return (
    <div className={`hidden md:block bg-zinc-900 border border-zinc-800 rounded-[10px] p-6 lg:p-10 shadow-2xl overflow-hidden relative ${className}`}>
      {/* Background Pattern */}
      ...
      <div className="flex flex-col md:flex-row items-center justify-between relative gap-6 md:gap-0">
        <div className="absolute top-[22px] lg:top-[45px] left-[calc(100%/12)] right-[calc(100%/12)] h-1 bg-zinc-800/50 hidden md:block">
          <div className="h-full bg-emerald-500 transition-all duration-1000 shadow-[0_0_15px_rgba(16,185,129,0.5)]" style={{ width: `${progressWidth}%` }}></div>
        </div>

        {steps.map((step, idx) => {
          const isCompleted = isCancelled ? step.step < cancelledStep : currentStep > step.step || (normalizedStatus === "completed" && step.step === 6);
          ...
          return (
            <div key={step.id} className="flex flex-row md:flex-col items-center gap-4 md:gap-4 relative z-10 flex-1 w-full md:w-auto">
              <div
                className={`w-[45px] h-[45px] lg:w-[90px] lg:h-[90px] rounded-[10px] flex items-center justify-center transition-all duration-700 border-2 lg:border-4 shrink-0 ${
                  isCompleted ? "bg-emerald-500 border-emerald-400/20 text-zinc-950" : isComplainedStep || isCancelledStep ? "bg-red-500 border-red-400/20 text-white" : isActive ? "bg-zinc-950 border-emerald-500 text-emerald-500" : "bg-zinc-950 border-zinc-800 text-zinc-700"
                }`}
              >
                {isCompleted ? (
              ...
              <div className="text-left md:text-center space-y-1">
                <p className={`text-[10px] lg:text-[10px] font-black uppercase tracking-[0.2em] ${isCancelledStep ? "text-red-500" : isActive || isCompleted ? "text-emerald-500" : isComplainedStep ? "text-red-500" : "text-zinc-300"}`}>{isComplainedStep ? "Komplain" : isCancelledStep ? "Batal" : step.label}</p>
                {(isActive || isCompleted || isComplainedStep || isCancelledStep) && (
                  <div className={`hidden md:flex px-3 py-1 ${isComplainedStep || isCancelledStep ? "bg-red-500/10 border-red-500/20" : "bg-emerald-500/10 border-emerald-500/20"} rounded-[10px] items-center justify-center gap-1.5 border`}>
                    <span className={`w-1.5 h-1.5 ${isComplainedStep || isCancelledStep ? "bg-red-500" : "bg-emerald-500"} rounded-[10px]`}></span>
                    <span className={`text-[8px] font-black uppercase tracking-tighter ${isComplainedStep || isCancelledStep ? "text-red-500" : "text-emerald-500"}`}>{normalizedStatus === "completed" ? "Transaksi Selesai" : isComplainedStep ? "Dalam Komplain" : isCancelledStep ? "Batal di Tahap Ini" : "Proses Aktif"}</span>
                  </div>
                )}
              </div>
