...
module.exports.login = async (req, res, next) => {
  try {
      ...
      const timeoutTimer = setTimeout(() => {
        ...
        if (attempt) {
          if (io) {
            ...
            // Update active session map on timeout auto-login
    ...
    // Direct login path: immediately update global active sessions map
    if (!global.activeSessions) {
      global.activeSessions = new Map();
    }
    global.activeSessions.set(user.id, sessionId);
