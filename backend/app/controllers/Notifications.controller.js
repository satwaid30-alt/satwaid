...
module.exports.markCommunityAsRead = async (req, res, next) => {
  try {
    ...
    await models.notifications.update({ is_read: true }, { where: { user_id, type: "community", is_read: false } });
    ...
    if (topicIds.length > 0) {
      await models.comments.update({ is_read: true }, { where: { topic_id: { [Op.in]: topicIds }, user_id: { [Op.ne]: user_id } } });
    }
...
module.exports.markAllAsRead = async (req, res, next) => {
  try {
    ...
    await models.notifications.update({ is_read: true }, { where: { user_id, is_read: false } });
