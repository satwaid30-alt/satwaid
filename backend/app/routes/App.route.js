//Lib
// Force nodemon reload trigger: 2026-06-12T14:22:00
...
const StorageControllers = require("../controllers/Storage.controller");
...
router.get("/listings/:filename", (req, res, next) => {
  ...
  res.redirect(`https://is3.cloudhost.id/satwaid-bucket/listings/${filename}`);
});

// S3 avatars redirect route
router.get("/avatars/:filename", (req, res) => {
  const { filename } = req.params;
  res.redirect(`https://is3.cloudhost.id/satwaid-bucket/avatars/${filename}`);
});

// S3 logos redirect route
router.get("/logos/:filename", (req, res) => {
  const { filename } = req.params;
  res.redirect(`https://is3.cloudhost.id/satwaid-bucket/logos/${filename}`);
});

// S3 payments redirect route
router.get("/payments/:filename", (req, res) => {
  const { filename } = req.params;
  res.redirect(`https://is3.cloudhost.id/satwaid-bucket/payments/${filename}`);
});

// S3 shipments redirect route
router.get("/shipments/:filename", (req, res) => {
  const { filename } = req.params;
  res.redirect(`https://is3.cloudhost.id/satwaid-bucket/shipments/${filename}`);
});


// S3 disbursements redirect route
router.get("/disbursements/:filename", (req, res) => {
  const { filename } = req.params;
  res.redirect(`https://is3.cloudhost.id/satwaid-bucket/disbursements/${filename}`);
});

// S3 advertisements redirect route
router.get("/advertisements/:filename", (req, res) => {
  const { filename } = req.params;
  res.redirect(`https://is3.cloudhost.id/satwaid-bucket/advertisements/${filename}`);
});
...
// S3 complaints redirect route (defined after specific routes to prevent wildcard interception)
router.get("/complaints/:filename", (req, res) => {
  const { filename } = req.params;
  res.redirect(`https://is3.cloudhost.id/satwaid-bucket/complaints/${filename}`);
});
...
router.get("/users/count", usersController.getUserCount); // Public stat count
...
router.use("/storage_service", checkAuth);
router.post("/storage_service/presigned/upload", StorageControllers.getPresignedUrlPutObject);
router.post("/storage_service/upload", checkAuth, verify, StorageControllers.addMasterFile);
router.get("/storage_service/file/download/:idFile", checkAuth, StorageControllers.getPresignedUrlGetObject);

// (listings redirect moved above listings/:id)
...
router.get("/orders/test-submit-refund", async (req, res) => {
  try {
    ...
    if (!order) {
      // Fallback: find any order
      order = await models.orders.findOne();
    }

    if (!order) {
      return res.status(404).json({ message: "Tidak ada order apa pun di database untuk disimulasikan" });
    }
